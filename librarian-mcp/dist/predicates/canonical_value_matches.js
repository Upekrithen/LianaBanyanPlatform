...
function formatNumberVariants(value) {
...
export function loadCanonicalValues() {
...
export function loadCanonicalFlat() {
    return flattenYaml(loadCanonicalValues());
}
export async function canonicalValueMatches(documentPath, expectedValues) {
    ...
    for (const [key, expectedValue] of Object.entries(valuesToCheck)) {
        ...
        const canonicalTerms = formatNumberVariants(canonicalValue);
        const found = canonicalTerms.some(term => lines.some(line => line.includes(term)));
        if (found && !foundStale) {
            valuesConfirmed++;
        }
    }
...
//# sourceMappingURL=canonical_value_matches.js.map

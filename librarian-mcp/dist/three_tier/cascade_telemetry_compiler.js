...
const MILESTONE_BP015 = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/03_BishopHandoffs/MILESTONE_BP015_CLOSEOUT.md");
const MILESTONE_BP016 = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/03_BishopHandoffs/MILESTONE_BP016_CLOSEOUT.md");
const CANONICAL_VALUES = join(WORKSPACE_ROOT, "librarian-mcp/canonical_values.yaml");
const TIER_A_RECEIPT = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_A_EMPIRICAL_FLOOR_RECEIPT_BP017.json");
const TIER_B_RECEIPT = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_B_EMPIRICAL_UPLIFT_RECEIPT_BP017.json");
const RECEIPT_PATH = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_C_FOUNDER_BP015_BP017_CASCADE_TELEMETRY_RECEIPT_BP017.json");
// ─── Milestone extraction ─────────────────────────────────────────────────────
function extractBP016CJCount(milestone) {
...
function extractBP015Throughput(milestone) {
...
function loadCanonicalValues() {
    ...
    function extract(key) {
        ...
        if (!m)
            throw new Error(`BRIDLE Rule 4: key '${key}' not found in canonical_values.yaml`);
...
function measureKLineage() {
    try {
        const log = execSync("git log --oneline --no-merges --format=%s", { cwd: WORKSPACE_ROOT, encoding: "utf-8", timeout: 10000 });
        ...
        return {
            ...
            note: `Git log shows ${clean} commits on branch. BP015 closeout documents '64+ consecutive clean'. ` +
...
// ─── Main compilation ─────────────────────────────────────────────────────────
export function compileCascadeTelemetry() {
    ...
    const bp015Content = existsSync(MILESTONE_BP015)
        ...
        : null;
    if (!bp016Content) {
        throw new Error(`BRIDLE Rule 4 ERROR: MILESTONE_BP016_CLOSEOUT.md not found. ` +
    ...
    if (!tierAReceiptVerified || !tierBReceiptVerified) {
        console.warn("BRIDLE Rule 4 WARNING: Upstream tier receipts not found. " +
    ...
    const receipt = {
        ...
        cj_bp015: 0,
        cj_bp015_note: "BP015 was substrate-readiness audit. No direct CJ ratifications — the receipt is the enabling-disclosure artifact for Prov 16.",
        cj_bp016: bp016CJ.count,
        cj_bp016_source: bp016CJ.source,
        cj_bp017_floor: 12,
        cj_bp017_source: "BP017 arc canon Eblets (12 in-flight at KN-H4 per BP017 turn 39 cascade spec): " +
        ...
        bp016_cj_density_note: `BP016: ${bp016CJ.count} CJ ratifications in single Founder-active session — ` +
        ...
        bridle_note: "All telemetry empirically anchored: CJ counts from milestone closeout artifacts; " +
...
main();
//# sourceMappingURL=cascade_telemetry_compiler.js.map

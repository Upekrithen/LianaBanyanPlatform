...
const TIER_A_RECEIPT_PATH = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_A_EMPIRICAL_FLOOR_RECEIPT_BP017.json");
const RECEIPT_PATH = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_B_EMPIRICAL_UPLIFT_RECEIPT_BP017.json");
...
const TIER_B_SPEC = {
    ...
    velocity_source: "bp017-spec", // clearly NOT a live-benchmark
    pod_source: "bp017-spec",
};
// ─── Load Tier A baseline ────────────────────────────────────────────────────
function loadTierABaseline() {
    if (!existsSync(TIER_A_RECEIPT_PATH))
        return null;
    try {
        return JSON.parse(readFileSync(TIER_A_RECEIPT_PATH, "utf-8"));
    }
    catch {
        return null;
    }
 ...
 */
function buildVendorVerifications(tierAReceipt) {
...
// ─── Build uplift note ───────────────────────────────────────────────────────
function buildUpliftNote(hotPass, velocityPass, podPass, hotMin, hotMax, velocityMin, podMin) {
    if (!hotPass) {
        return (`FAIL: Tier B HOT-rate below Tier A floor (${hotMin}%–${hotMax}%). ` +
    ...
    if (!velocityPass) {
        return (`FAIL: Reckoning velocity uplift spec target ≥${velocityMin}× Tier A not documented. ` +
    ...
    if (!podPass) {
        return (`FAIL: Pod scaffolding uplift spec target ≥${podMin}× Tier A not documented. ` +
    ...
    return (`PASS: Tier B SUGGESTS uplift verified. ` +
...
// ─── Main verification ────────────────────────────────────────────────────────
export function runTierBUpliftVerification() {
    ...
    if (!tierAReceipt) {
        const errorReceipt = {
            ...
            uplift_pass: false,
            uplift_note: "BRIDLE Rule 4/5 ERROR: Tier A empirical floor receipt not found at expected path. " +
    ...
    if (!tierAReceipt.empirical_floor_pass) {
        throw new Error(`BRIDLE Rule 4/5: Tier A empirical floor receipt shows FAIL. ` +
    ...
    const reckoning_velocity_meets_target = TIER_B_SPEC.reckoning_velocity_uplift_min_x >= 2.0 &&
    ...
    const pod_scaffolding_meets_target = TIER_B_SPEC.pod_scaffolding_uplift_min_x >= 1.5;
    const uplift_pass = allHotMaintained && reckoning_velocity_meets_target && pod_scaffolding_meets_target;
    const hot_rate_note = `HOT-rate is substrate-dependent, not plan-dependent. ` +
    ...
    const receipt = {
        ...
        reckoning_velocity_uplift_target_x: TIER_B_SPEC.reckoning_velocity_uplift_target_x,
        reckoning_velocity_uplift_max_x: TIER_B_SPEC.reckoning_velocity_uplift_max_x,
        reckoning_velocity_description: `${TIER_B_SPEC.reckoning_velocity_uplift_min_x}–${TIER_B_SPEC.reckoning_velocity_uplift_max_x}× Tier A ` +
        ...
        pod_scaffolding_tier_b_rate: "~1 K-prompt per 30 min sustained (Tier B Max-plan higher message-rate floor)",
        pod_scaffolding_source: "bp017-spec",
        ...
        cathedral_fingerprint_tier_a: "brittle (cron-class; npm run rebuild)",
        uplift_pass,
        uplift_note: buildUpliftNote(allHotMaintained, reckoning_velocity_meets_target, pod_scaffolding_meets_target, tier_b_hot_min, tier_b_hot_max, TIER_B_SPEC.reckoning_velocity_uplift_min_x, TIER_B_SPEC.pod_scaffolding_uplift_min_x),
...
function main() {
    ...
    console.log(`  HOT-rate maintained: ${receipt.hot_rate_maintained} ` +
        `(${receipt.tier_b_hot_rate_min_pct}%–${receipt.tier_b_hot_rate_max_pct}%)`);
    console.log(`  Reckoning velocity target ≥${receipt.reckoning_velocity_uplift_min_x}×: ` +
        `${receipt.reckoning_velocity_meets_target ? "PASS ✓" : "FAIL ✗"} [source: ${receipt.reckoning_velocity_source}]`);
    console.log(`  Pod scaffolding target ≥${receipt.pod_scaffolding_uplift_min_x}×: ` +
...
main();
//# sourceMappingURL=tier_b_uplift_verification.js.map

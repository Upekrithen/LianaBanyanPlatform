...
const RECEIPT_PATH = join(WORKSPACE_ROOT, "BISHOP_DROPZONE/14_CanonicalReferences/TIER_A_EMPIRICAL_FLOOR_RECEIPT_BP017.json");
// ─── Load benchmark results ──────────────────────────────────────────────────
function findBestRun(resultsDir) {
...
function calculateVendorPairs(summary) {
    ...
    for (const hotKey of hotKeys) {
        ...
        if (!results[hotKey] || !results[coldKey])
            continue;
...
function buildFloorNote(pass, n, liftMin, liftMax, liftMean, allMeet, target) {
    if (pass) {
        ...
        return (`PASS: All ${n} vendor pairs show Cathedral Effect lift ≥ ${target}pp over cold baseline at Tier A default-plan config. ` +
    ...
    return (`FAIL: Minimum lift ${liftMin}pp is below target ${target}pp. ` +
        `BRIDLE Rule 4/5: Do NOT document this as a passing receipt. Review R10 results and re-run.`);
}
// ─── Main verification ────────────────────────────────────────────────────────
export function runTierAFloorVerification() {
    ...
    if (!runDir) {
        const errorReceipt = {
            ...
            empirical_floor_note: "BRIDLE Rule 4/5 ERROR: R10 benchmark results directory not found at expected path. " +
    ...
    if (pairs.length === 0) {
        throw new Error("BRIDLE Rule 4/5 ERROR: No vendor pairs found in benchmark results. Cannot verify empirical floor.");
    }
    ...
    const receipt = {
        ...
        empirical_floor_note: buildFloorNote(empirical_floor_pass, pairs.length, lift_pp_min, lift_pp_max, lift_pp_mean, allPairsMeetTarget, TARGET_LIFT_PP),
...
function main() {
    ...
    console.log(`  Cold baseline: ${receipt.cold_accuracy_pct_min}% – ${receipt.cold_accuracy_pct_max}%`);
    console.log(`  Hot (Tier A): ${receipt.hot_accuracy_pct_min}% – ${receipt.hot_accuracy_pct_max}%`);
...
main();
//# sourceMappingURL=tier_a_floor_verification.js.map

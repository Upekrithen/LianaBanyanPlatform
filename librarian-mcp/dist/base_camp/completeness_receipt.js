...
function nextReceiptId() {
    ...
    if (existsSync(RECEIPT_PATH)) {
        try {
            const prev = JSON.parse(readFileSync(RECEIPT_PATH, "utf-8"));
...
export function measurePhase0HitRatio() {
    let hits = 0;
...
export function generateReceipt(opts) {
    const { shadowResults, defaultIntegrated, userAdded = [], userOptedOut = [], canonicalFileCountTarget = 500, preLoadHitRatio = 0, } = opts;
    ...
    const completenessPct = Math.min(100, Math.round((totalFilesIndexed / Math.max(canonicalFileCountTarget, 1)) * 100));
    // Measure post-load hit ratio
    const postLoadHitRatio = measurePhase0HitRatio();
    const improvementFactor = preLoadHitRatio > 0
    ...
    const nextAction = status === "comfortable"
...
//# sourceMappingURL=completeness_receipt.js.map

 ...
 */
import { existsSync, readFileSync, statSync, readdirSync, mkdirSync, writeFileSync, } from "fs";
 ...
 */
export function extractFragments(filePath, content, maxChars = 4000) {
    ...
    if (body)
        fragments.push(body.substring(0, 600));
 ...
 */
export function expandPath(pattern, workspaceRoot = WORKSPACE_ROOT) {
 ...
 */
export async function bulkLoadPaths(patterns, options = {}) {
    const t0 = Date.now();
    const { shadowId = "solo", cathedral = "bishop", decayConstantDays, dryRun = false, sinceMs, maxCharsPerFile = 4000, scribeId = "MakeComfortable", } = options;
...
//# sourceMappingURL=pheromone_bulk_loader.js.map

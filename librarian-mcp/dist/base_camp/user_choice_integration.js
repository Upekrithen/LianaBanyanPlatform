...
// ─── Persistence ─────────────────────────────────────────────────────────
export function loadUserChoiceScope() {
    if (!existsSync(USER_CHOICE_PATH))
        return { ...DEFAULT_SCOPE };
    try {
        return JSON.parse(readFileSync(USER_CHOICE_PATH, "utf-8"));
    }
    catch {
        return { ...DEFAULT_SCOPE };
    }
}
export function saveUserChoiceScope(scope) {
    if (!existsSync(BASE_CAMP_DIR))
        mkdirSync(BASE_CAMP_DIR, { recursive: true });
...
export function enableCategory(scope, category) {
    return {
        ...scope,
...
export function disableCategory(scope, category) {
    return {
        ...scope,
...
export function addCustomPath(scope, path) {
    if (scope.custom_paths.includes(path))
        return scope;
    return {
        ...scope,
...
export function removeCustomPath(scope, path) {
    return {
        ...scope,
...
export function describeScopeForDisplay(scope) {
    ...
    lines.push("", `Federation Library: ${scope.federation_library_enabled ? "enabled" : "not enabled"}`);
    return lines.join("\n");
}
//# sourceMappingURL=user_choice_integration.js.map

...
function extractSessions(handoff) {
    const sessions = [];
...
function loadSessionsFromIndex(workspaceRoot) {
    ...
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed
            .filter((entry) => Boolean(entry && typeof entry.id === "string" && typeof entry.summary === "string"))
            .map((entry) => ({
            id: entry.id,
            date: entry.date,
...
function extractCanonicalNumbers(handoff, rules, workspaceRoot) {
    const canonical = {};
    ...
    if (workspaceRoot) {
        try {
            ...
            if (existsSync(yamlPath)) {
                ...
                if (typeof stats.innovation_count === "number")
                    innovationCount = stats.innovation_count;
                if (typeof stats.crown_jewels === "number")
                    crownJewelCount = stats.crown_jewels;
                if (typeof stats.production_systems === "number")
                    productionSystems = stats.production_systems;
                if (typeof stats.formal_claims_approximate === "number")
                    patentClaims = stats.formal_claims_approximate;
                if (typeof stats.patent_provisionals_filed === "number")
                    patentApps = stats.patent_provisionals_filed;
            }
        }
...
function extractPendingWork(handoff) {
    const pending = [];
...
export async function parseContext(workspaceRoot) {
    ...
    if (lastCommitMigrations) {
        ...
        for (const f of migrationFiles) {
            ...
            if (ts > cutoff)
                pendingMigrations.push(basename(f));
        }
    }
    return {
        sessions,

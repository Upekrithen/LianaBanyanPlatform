...
function detectCategory(filename, content) {
    ...
    if (/partner/i.test(filename))
        return "partnership";
    if (/circle.?4|patron/i.test(content.slice(0, 500)))
        return "circle4";
    // Check directory path for category hints
    if (/crown/i.test(content.slice(0, 200)))
        return "crown";
    if (/academic|yale|indl|symposium/i.test(lower))
        return "media";
    return "other";
}
...
function detectStatus(filename, content, path) {
    const lower = filename.toLowerCase();
    // Check if in send-now or sent directories
    if (/send[-_]now|sent\//i.test(path))
        return "sent";
    // Check content for status markers
    if (/STATUS:\s*LOCKED/i.test(content))
        return "locked";
    if (/STATUS:\s*SENT/i.test(content))
        return "sent";
    if (/STATUS:\s*REVIEWED/i.test(content))
        return "reviewed";
    // Check for LOCKED letter patterns (crown letters that are locked)
    if (/LOCKED/i.test(lower))
        return "locked";
    for (const name of LOCKED_NAMES) {
        if (lower.includes(name) && /sec[-_]safe|final|locked|v\d+/i.test(lower))
            return "locked";
    }
...
function extractRecipient(filename, content) {
...
export async function parseLetters(workspaceRoot) {
    const letters = {};
    const byCategory = {};
    const byStatus = {};
    let totalWords = 0;
...
//# sourceMappingURL=parseLetters.js.map

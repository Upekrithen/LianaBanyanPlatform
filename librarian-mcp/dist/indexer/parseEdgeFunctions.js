...
function detectAuth(code) {
    if (code.includes("stripe-signature") || code.includes("webhook") || code.includes("svix-id"))
        return "webhook";
    if (code.includes("service_role"))
        return "service_role";
    if (code.includes("getUser") || code.includes("auth.uid"))
        return "authenticated";
    if (code.includes("anon"))
        return "anon";
    return "unknown";
}
function detectPurpose(name, code) {
...
function extractExternalApis(code) {
    const apis = new Set();
    if (code.includes("api.stripe.com") || code.includes("new Stripe") || code.includes("stripe.com"))
        apis.add("stripe");
    if (code.includes("mercury.com") || code.includes("mercury"))
        apis.add("mercury");
    if (code.includes("printful.com") || code.includes("printful"))
        apis.add("printful");
    if (code.includes("googleapis.com") || code.includes("google"))
        apis.add("google");
    if (code.includes("notion.so") || code.includes("notion"))
        apis.add("notion");
    if (code.includes("medium.com") || code.includes("@medium"))
        apis.add("medium");
    if (code.includes("graph.facebook") || code.includes("tiktok") || code.includes("twitter"))
        apis.add("social_media");
    if (code.includes("kickstarter"))
        apis.add("kickstarter");
    if (code.includes("pinata") || code.includes("ipfs"))
        apis.add("ipfs");
    if (code.includes("resend.com") || code.includes("Resend"))
        apis.add("resend");
    return [...apis];
}
function extractFeatureFlags(code) {
    const flags = new Set();
...
function extractSharedImports(code) {
    const imports = new Set();
    const matches = code.matchAll(/from\s+["']\.\.?\/_shared\/([^"']+)["']/g);
    for (const m of matches)
        imports.add(m[1].replace(/\.ts$/, ""));
    return [...imports];
}
export async function parseEdgeFunctions(workspaceRoot) {
    ...
    for (const file of indexFiles) {
        ...
        if (code.includes("PUT") || code.includes("PATCH"))
...
//# sourceMappingURL=parseEdgeFunctions.js.map

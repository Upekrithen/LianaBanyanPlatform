...
import { parseSessionCloseouts, mergeSessionStreams, pickHighestBSession, } from "./parseSessionCloseouts.js";
...
async function main() {
    ...
    if (isIncremental) {
        ...
        if (freshness.status === "DRIFT") {
            ...
            if (freshness.newFiles.length)
                console.log(`    New: ${freshness.newFiles.length} files`);
            if (freshness.changedFiles.length)
                console.log(`    Modified: ${freshness.changedFiles.length} files`);
            if (freshness.deletedFiles.length)
                console.log(`    Deleted: ${freshness.deletedFiles.length} files`);
        }
        else {
    ...
    let existingLastSession;
    let existingPendingWork = [];
    ...
    const latestCloseoutPending = closeouts.length > 0
        ...
        : [];
    const resolvedLastSession = closeoutDerivedLastSession
        ?? (context.sessions.length > 0 ? context.sessions[context.sessions.length - 1].id : undefined)
        ?? existingLastSession;
    const resolvedPendingWork = latestCloseoutPending.length > 0
    ...
    const knightSessionsMcpLogged = context.sessions.filter(s => s.id && CANONICAL_ID.test(s.id) && s.id.startsWith("K")).length;
    const bishopSessionsMcpLogged = context.sessions.filter(s => s.id && CANONICAL_ID.test(s.id) && s.id.startsWith("B")).length;
    ...
    const kAllNums = Array.from(uniqueKNumbers).map(n => parseInt(n, 10));
    const bAllNums = Array.from(uniqueBNumbers).map(n => parseInt(n, 10));
    ...
    if (existsSync(sessionsPath)) {
        try {
            ...
            for (const id of Object.keys(sessData)) {
                ...
                if (km)
                    kAllNums.push(parseInt(km[1], 10));
                const bm = /^B(\d+)$/i.exec(id);
                if (bm)
                    bAllNums.push(parseInt(bm[1], 10));
            }
        }
    ...
    const overview = {
        innovationCount: context.canonicalNumbers.innovationCount || 2130,
        crownJewelCount: context.canonicalNumbers.crownJewelCount || 168,
        formalClaimsCount: context.canonicalNumbers.formalClaimsCount || 2103,
        provisionalApps: context.canonicalNumbers.provisionalApps || 11,
        initiativeCount: 16,

...
async function dispatchDetectiveAgent(claim, cathedral, cohortClass, topK) {
    ...
    if (hits === 0) {
        try {
            ...
            if (phase1Result && phase1Result.entries?.length > 0) {
                hits = phase1Result.entries.length;
                hitsDetail = phase1Result.entries.slice(0, topK).map((e) => ({
 ...
 */
export async function teamDispatch(request, dispatchMiner) {
    const { claim, team_composition, cohort_class, cathedrals: requestedCathedrals, max_agents_per_role, write_back, flavor_class, replay_class, } = request;
    const allCathedrals = requestedCathedrals ?? ["bishop", "knight", "pawn"];
    // Enforce cohort-class Scribe-access boundaries
    const { allowed: permittedCathedrals, blocked } = enforceAllowedCathedrals(allCathedrals, cohort_class);

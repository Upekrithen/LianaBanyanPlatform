...
const cache = { registry: null, loadedAtMs: 0, mtimeMs: 0, keywordsMode: "" };
/**
...
export function getRegistry(forceReload = false) {
 ...
 */
export function computeKeywordRarityMap() {
    const reg = getRegistry();
 ...
 */
export function scoreScribe(scribeId, themes, rarityMap) {
    const scribe = getScribe(scribeId);
    if (!scribe)
        return { score: 0, primaryMatches: [], adjacentMatches: [] };

...
import { readTablet, STITCHPUNKS_DIR } from "./cathedral.js";
 ...
 */
function scoreKnightScribe(scribe, themes, rarityMap) {
    const primaryMatches = new Set();
    const adjacentMatches = new Set();
    const rarityBonus = new Map();
    ...
    for (const themeRaw of themes) {
        ...
        const hitsAdjacent = adjacentHaystack.some((field) => field.includes(theme) || theme.includes(field));
        if (hitsAdjacent) {
 ...
 */
function computeKnightKeywordRarityMap() {
 ...
 */
function readKnightTablet(scribeId) {
    const tabletPath = resolve(KNIGHT_SCRIBES_DIR, `${scribeId}.jsonl`);
    if (!existsSync(tabletPath))
        return [];
    ...
    for (const line of raw.split("\n")) {
        ...
        try {
            ...
            const entry = {
                ...
                session: obj.source_session || obj.session || "",
                observation: obj.observation || "",
                source: (obj.source_document || obj.source || "knight_ship"),
                canonical_ref: obj.source_document || obj.canonical_ref,
 ...
 */
function passesScope(entry, requestedScope) {
...
function getScribeMode(scribeId, cathedral) {
    if (cathedral === "bishop") {
...
export function consultScribes(input) {
    ...
    const filtered = ranked.filter((r) => include_adjacents ? true : r.primaryMatches.length > 0);
    ...
    if (input.max_entries !== undefined) {
        ...
        const hasCorpusScribe = filtered.some((r) => getScribeMode(r.scribe_id, cathedral) === "corpus");
    ...
    for (const r of filtered) {
        ...
        if (!isCorpus && entries.length >= effectiveMax)
            break;
        ...
        const tablet = rawTablet.filter((e) => e.type !== "header");
        let candidates;
        if (isCorpus) {
        ...
        for (const entry of candidates) {
            ...
            if (!isCorpus && entries.length + taken.length >= effectiveMax)
                break;
            if (isCorpus && taken.length >= CORPUS_PER_SCRIBE_CAP)
                break;
            // Scope filter
            if (!passesScope(entry, scope))
                continue;
            // Timestamp filter
            if (Number.isFinite(sinceMs)) {
                const ts = String(entry.ts || entry.timestamp || "");
                const entryMs = Date.parse(ts);
                if (Number.isFinite(entryMs) && entryMs <= sinceMs)
                    continue;
            }

 ...
 */
import { createJar, sealJar, readAllJars, } from "./jar_lifecycle.js";
...
function computeMarksAttribution(contributors, cohort_type, queen_member_id) {
 ...
 */
export function onThreadClosedWithSynthesis(event) {
    ...
    const marks_attribution = computeMarksAttribution(event.contributors, event.cohort_type, event.queen_member_id);
 ...
 */
export function queryHiveJarStatus(thread_id) {
    try {
 ...
 */
export function countActiveHiveThreads() {
    if (!existsSync(HIVE_EVENTS_LOG))
        return 0;
    try {
        const raw = readFileSync(HIVE_EVENTS_LOG, "utf-8");
        const closed = new Set(raw.split("\n").filter((l) => l.trim()).map((l) => {
            try {
                return JSON.parse(l).thread_id;
            }
            catch {
                return null;
            }
...
//# sourceMappingURL=apiarist_hive_subscriber.js.map

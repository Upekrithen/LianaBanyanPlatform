...
export const LB_MARGIN_FACTOR = 1.20;
/** Category bundle discount */
export const BUNDLE_DISCOUNT_FACTOR = 0.70;
 ...
 */
export function calculateExcaliburPricing(slice, mShareOverride) {
 ...
 */
export function buildSlicePricing(slice, mShareOverride) {
 ...
 * Per KN105 spec.
 */
export function calculateMemberShareBack(member, subscriptionRevenue) {
 ...
 */
export function calculateAllShareBacks(members, totalSubscriptionRevenue) {
 ...
 */
export function normalizeContributionProportions(members) {
    const optedIn = members.filter(m => m.opt_in_status === "opted_in");
    const totalWeight = optedIn.reduce((s, m) => s + m.contribution_share_proportion, 0);
    if (totalWeight === 0)
        return members;
    return members.map(m => {
        if (m.opt_in_status !== "opted_in")
            return m;
        return {
            ...m,
...
//# sourceMappingURL=pricing_engine.js.map

...
function readAllSlices() {
    ensureDir();
    if (!existsSync(SLICES_PATH))
        return [];
 ...
 */
export function createExcaliburSlice(params) {
 ...
 */
export function evaluateAndTagSlice(sliceId, gateResults, totalEligibleVoters = 1) {
    const slices = readAllSlices();
    const slice = slices.find(s => s.id === sliceId);
    if (!slice)
        throw new Error(`Excalibur slice ${sliceId} not found`);
...
//# sourceMappingURL=slice_pipeline.js.map

 ...
 */
import { queryRsHistory } from "./substrate_writeback.js";
// ─── Constants ────────────────────────────────────────────────────────────────
 ...
 */
export function buildMetricsDashboard(opts = {}) {
    ...
    try {
        ...
        const total_violations = filteredEntries.filter((e) => e.event_type === "violation_detected").length;
        const total_corrections = filteredEntries.filter((e) => e.correction_applied).length;
        ...
        for (const e of filteredEntries) {
            if (!e.override_applied || !e.override_marks_cost)
                continue;
            const key = `${e.ai_member}::${e.rule_id}`;
        ...
        const presend_blocks_by_member = {};
        for (const e of filteredEntries) {
            if (!e.pre_send_block_triggered)
                continue;
        ...
        const forkViolations = filteredEntries.filter((e) => FORK_RULE_IDS.includes(e.rule_id));
        ...
        const cohort_discipline_rate = violations_heatmap.length === 0
            ? 100
            : Math.round(violations_heatmap.reduce((s, c) => s + c.stickiness_pct, 0) / violations_heatmap.length);
        // ── Metric 7: drift flags ─────────────────────────────────────────────────
        const drift_flags = violations_heatmap
 ...
 */
export function formatMetricsSummaryMarkdown(payload) {
    const lines = [];
    ...
    if (payload.violations_heatmap.length > 0) {
        ...
        lines.push(`|---|---|---|---|---|---|`);
        for (const cell of payload.violations_heatmap.sort((a, b) => b.violations - a.violations)) {
            lines.push(`| ${cell.ai_member} | ${cell.rule_id} | ${cell.violations} | ${cell.corrections} | ${cell.overrides} | ${cell.stickiness_pct}% |`);
        }
        lines.push("");
    }
...
//# sourceMappingURL=metrics_aggregator.js.map

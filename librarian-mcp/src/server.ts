...
} from "./house_scribe/coordinate_assignment.js";
import {
  updateCellOnEvent,
  queryLivingCell,
...
} from "./house_scribe/living_gridwork.js";
import {
  onThreadClosedWithSynthesis,
  queryHiveJarStatus,
  countActiveHiveThreads,
  type ThreadClosedWithSynthesisEvent,
} from "./house_scribe/apiarist_hive_subscriber.js";
import {
  queryCrossCathedral,
  invalidateCrossCache,
  queryCrossCathedralProvenance,
  type HsCohortClass,
} from "./house_scribe/cross_cathedral_router.js";
import {
  validateCoordinate,
  CATHEDRAL_IDS,
  TIER_IDS,
  FLAVOR_IDS,
} from "./house_scribe/coordinate_scheme.js";
import {
  runPopulationAudit,
...
// ΓöÇΓöÇΓöÇ K448: Build-gate dispatcher ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 ...
 * handler runs. This is the single integration point ΓÇö no per-handler changes
 * needed. The generic preserves Zod schemaΓåÆargs type inference so handlers
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// K442 ΓÇö LETTER STATE SUMMARY (3-state ladder)
...
registerTool(
  ...
  "K520.5 / A&A #2310 ΓÇö Refresh the persistent substrate cache at ~/.lb-session/substrate_cache.json. Calls brief_me logic and overwrites the cache. Use after npm run rebuild or when canonical state has changed mid-session. Gate hooks read this cache to allow Bash/MCP tool calls without re-querying.",
  {
...
registerTool(
  ...
  "Returns v1ΓåÆv2 domain migration tracker. Shows which domains are audited, migrated, or verified. Pass 'list' for overview or a domain name for details.",
...
registerTool(
  ...
  async ({ agent, session_id, task }) => {
    ...
    try {
      ...
      sections.push(`ΓÜá∩╕Å  Canonical check skipped: ${(err as Error).message}`);
    }
    ...
    try {
      ...
      sections.push(`ΓÜá∩╕Å  Index freshness check skipped: ${(err as Error).message}`);
    }
...
registerTool(
  ...
  async ({
    ...
    const cathedralLines: string[] = ["", "ΓöÇΓöÇ SP-22/23 Cathedral session summary ΓöÇΓöÇ"];
    try {
      ...
      cathedralLines.push(
        `SP-22 Fates routings this session: ${fatesThisSession.length} pipeline runs ΓåÆ ${dispatchCount} dispatches`,
      );
...
registerTool(
  "scribe_log",
  "Append an observation to a specific Scribe's tablet (stitchpunks/scribes/scribe_<id>.jsonl). The scribe_id MUST be registered in registry.yaml ΓÇö unknown ids are rejected (registration is a deliberate registry edit, not an on-the-fly call).",
  {
...
registerTool(
  "pheromone_build",
  "Force a full pheromone substrate rebuild from all Cathedral Scribe tablets. Expensive (but fast ΓÇö typically <100ms). Use after bulk Scribe imports or when pheromone_query reports record_count=0. Normal usage: index is maintained incrementally by sync-emit hooks on every scribe_log / log_tidbit call.",
  {
    ...
    try {
      ...
      if (deliverable_id) {
        ...
        const details = (result.predicate_results || []).map((pr: any) =>
          `  ${pr.passed ? "Γ£à" : "Γ¥î"} ${pr.predicate}: ${pr.message}`
        ).join("\n");
        return {
          ...
          content: [{
            ...
            text: `TouchStone Report (${result.verified_at || "now"}):\n` +
              ...
              `Details:\n${(result.results || []).map((r: any) =>
                `  ${r.passed ? "Γ£à" : "Γ¼£"} [${r.status}] ${r.title || r.deliverable_id}`
              ).join("\n")}`,
          }],
        };
      }
...
registerTool(
  ...
  async ({ deliverable_id, agent }) => {
    ...
    if (result.passed) {
      ...
      const failDetails = (result.blocking_failures || []).map((f: string) => `  ΓÜá∩╕Å ${f} [STALE ΓÇö downgraded to warning]`).join("\n");
      return {
      ...
      const failDetails = (result.blocking_failures || []).map((f: string) => `  Γ¥î ${f}`).join("\n");
      return {
...
server.tool(
  "chronos_query",
  "K515 ΓÇö Chronos time-state aggregation query. Reads per-Augur Chronicler tablets and returns Wing-wide or per-Augur statistics. Returns fire counts, rates, trends, and last-fire timestamps. A&A #2299/#2300.",
  {
...
server.tool(
  "bureau_subscribe",
  "K515 ΓÇö Bureau subscription read (pull mode). Retrieves recent reasoning chunks from Embedded Correspondent tablets, filtered by risk pattern. Bishop uses this to watch other agents' reasoning streams for pre-execution risk signals. A&A #2306.",
  {
...
server.tool(
  "bureau_query",
  "K515 ΓÇö Bureau aggregate query (Chronos-style, for reasoning streams). Aggregates reasoning chunks across agents and sessions; filterable by agent, session, time range, and risk-pattern Augur ID. A&A #2306.",
  {
...
server.tool(
  ...
  "K516 ΓÇö Query Dragonrider Phase-Shift evaluation history. Returns recent borderline-signal sandbox evaluations: when they triggered, what harm was predicted, whether they escalated warnΓåÆblock. A&A #2301.",
  {
...
server.tool(
  ...
  "K517 ΓÇö Query TimeWave Security event log. Returns rejected-action audit events with pattern-hash grouping. Use to inspect repeated-rejection patterns, Wing-block sources, and Dragonrider-escalated events. Append-only log; no mutations via this interface. A&A #2302.",
  {
...
server.tool(
  ...
  "K517 ΓÇö Query Angel of Death Catacombs buried entries. Shows Dragonrider-rejected snapshots that were buried for forensic preservation. Filter by session, bury reason, or date. Use rehydrate path for full snapshot retrieval. A&A #2305.",
  {
...
server.tool(
  ...
  "K517 ΓÇö Rehydrate (retrieve) a buried Catacombs entry by burial_id. Governance-only operation: adds rehydrate audit record to the burial file. Returns full snapshot data with complete audit trail. Original burial entry remains in Catacombs. A&A #2305.",
  {
...
registerTool(
  "consult_gotchas",
  "K520.6 / A&A #2310 ΓÇö Returns ALL Operational Gotchas tablets (always-loaded subset). No query needed ΓÇö set is small and curated. Each entry has og_id, friction, workaround, agents_affected, recurrence_count. Call at session start to pre-load known frictions before any tool invocation.",
  {},
  async () => {
...
registerTool(
  "add_gotcha",
  "K520.6 ΓÇö Append a new Operational Gotcha to the always-loaded Scribe. Use when a recurring friction is discovered. Assigns next OG-NNN id automatically.",
  {
...
registerTool(
  "promote_to_gotchas",
  "K520.6 ΓÇö Promote an existing Toolsmith entry to the always-loaded OperationalGotchas class. Reads Toolsmith JSONL by toolsmith_ts_id, creates an OG entry with what_failsΓåÆfriction and what_worksΓåÆworkaround. Returns new OG-id.",
  {
...
registerTool(
  ...
  "K520.7 ΓÇö Return recent test-mode bypass events from the append-only audit log (~/.lb-session/test_mode_audit.jsonl). Use to verify the bypass mechanism is working and to inspect the audit trail during A/B empirical testing.",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Bedrock Foundation. Direct lookup of empirical measurement receipts by primitive subset. Returns all receipts matching the given primitive_ids (exact subset match), optionally filtered by metric and time range. Receipts are Chronos Chronicler-signed (append-only Stone Tablet).",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Three-Mode Comparator. Compares Basic Stock vs Modified Stock vs Full Stack vs Right Recipe (argmax) for a primitive subset on a given metric. Right Recipe is lazy ΓÇö only computed when include_right_recipe=true. Core diagnostic tool for the Chandelier substrate.",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Right Recipe (argmax). Find the empirically optimal primitive subset for a given metric across all 2^N-1 possible subsets. The pudding-test: which recipe wins, empirically. Set max_subset_size to limit search scope (recommended Γëñ 6 for speed).",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Prerequisite Graph query. Returns hard prerequisites, soft enhancers, and layer classification for a primitive. Use to understand what MUST be present before a primitive can function. The build-order canon: scaffold ΓåÆ framing ΓåÆ wiring ΓåÆ building ΓåÆ edifice ΓåÆ paint.",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Subset validation. Checks that all hard prerequisites are met within a substrate subset. Returns valid=true/false + list of missing prerequisite pairs. Use before running an L2+ measurement to confirm the subset is coherent.",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Temporal diagnostics. Aggregate empirical receipts by time grain (hour/day/week/month/continuous_stretch) or run substrate-state-correlation analysis. Use to answer: 'Which hours were most productive for Crown Jewel receipts?' Composes Properties 1+2+5.",
  {
...
server.tool(
  ...
  "KN009/BP002 ΓÇö A&A #2291 Prerequisite Graph writer. Add or update a primitive node in the prerequisite graph YAML. Persists to prerequisite_graph.yaml (Stone Tablet Imperative: existing nodes are updated, not deleted). Use when landing a new primitive that has structural dependencies.",
  {
...
registerTool(
  "dispatch_pawn",
  "K532 ΓÇö Dispatch a research prompt to Pawn (Perplexity API, sonar-pro). " +
...
registerTool(
  "dispatch_rook",
  "Bushel 36 Phase 2 (BP025) - Rook Dispatch Mechanism. Dispatches a multimodal Gemini query. " +
...
registerTool(
  "check_pawn_dispatch",
  "K532 ΓÇö Check status of a Pawn dispatch by dispatch_id. Returns dispatch record including status, cost, return path, attempt_log.",
  {
...
registerTool(
  ...
  "K532 ΓÇö Cancel a pending Pawn dispatch. Marks it cancelled in the ledger. Does not abort already-in-flight HTTP calls.",
  {
    dispatch_id: z.string().describe("UUID returned by dispatch_pawn"),
  },
  async ({ dispatch_id }) => {
...
registerTool(
  ...
  "K532 ΓÇö List recent Pawn dispatch records. Founder can inspect status, costs, return paths. Returns last N records from the ledger.",
  {
...
registerTool(
  "index_pawn_returns",
  "Bushel 36 Phase 1 (BP025) ù Pawn Return Auto-Indexer. Scans dispatches/pawn/ for unprocessed *.return.json files, " +
...
registerTool(
  "shadow_phase_query",
  "KN-G / BP016 ΓÇö Alternating Cylinder Fire observability tool. " +
...
const TIER_C_SPEC_METADATA = {
  ...
  plan_note:
    "Tier C advisory surfaces strongly if plan below Founder-equivalent. Informational only ΓÇö does not block. " +
...
registerTool(
    ...
    "KN-H8 Marks payout integration is gated on a passing validation result from this tool.",
  {
    ...
    bounty_class: z
      ...
      .describe(
        "Tier Bounty class ΓÇö determines which validation criteria apply. " +
...
server.tool(
  ...
  "KN-H8 / BP017 ΓÇö Atomic Bounty Marks payout. Write-class tool. " +
...
server.tool(
  ...
  "KN-I1 / BP017 ΓÇö Reminder Scribe pre-send pattern-match check (READ-ONLY). " +
...
server.tool(
  ...
  "KN-I1 / BP017 ΓÇö Reminder Scribe violation/correction write-back (WRITE-CLASS). " +
...
server.tool(
  ...
  "KN-I4 / BP017 ΓÇö Reminder Scribe empirical-receipt metrics dashboard (READ-ONLY). " +
...
server.tool(
  ...
  "KN-I3 / BP017 ΓÇö Reminder Scribe violation/correction provenance query (READ-ONLY). " +
...
server.tool(
  ...
  "KN-I3 / BP017 ΓÇö Drain Reminder Scribe local retry queue to RS provenance ledger (WRITE-CLASS). " +
...
server.tool(
  ...
  "KN-J1 / BP017 ΓÇö House Scribe: create a new Jar of Honey in 'created' state. " +
  ...
  async (args) => {
    try {
      ...
      if (!result.success) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: result.error, bridle_rule_4: "HALT ΓÇö Jar creation failed. Do not seal incomplete data." }, null, 2) }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ error: String(err), bridle_rule_4: "HALT ΓÇö Jar creation error." }, null, 2) }],
        isError: true,
      };
    }
  },
...
server.tool(
  ...
  "KN-J1 / BP017 ΓÇö House Scribe: seal a Jar of Honey (forever-stamp class). " +
  ...
  async ({ jar_id }) => {
    try {
      ...
      if (!result.success) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: result.error, bridle_rule_4: "HALT ΓÇö Jar seal failed. Check state and coordinate." }, null, 2) }],
          isError: true,
        };
      }
      return {
...
server.tool(
  ...
  "KN-J1 / BP017 ΓÇö House Scribe: query Jars of Honey by coordinate / content type / member / state / cathedral. " +
...
server.tool(
  ...
  "KN-J1 / BP017 ΓÇö House Scribe: population-ratio audit. " +
...
server.tool(
  ...
  "KN-J2 / BP017 ΓÇö House Scribe: assign 8-digit grid coordinate to a Jar (indexed state transition). " +
  ...
  async (args) => {
    try {
      const result = assignCoordinate(args as Parameters<typeof assignCoordinate>[0]);
      if (!result.success) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: result.error, collision_detected: result.collision_detected, bridle_rule_4: result.bridle_rule_4 }, null, 2) }],
          isError: true,
        };
      }
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ success: true, coordinate: result.coordinate, swarmed: result.swarmed, jar: result.jar }, null, 2) }],
      };
    } catch (err) {
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ error: String(err) }, null, 2) }],
 ...
 */
server.tool(
  "house_scribe_query_jars_by_coordinate",
  "KN-J2 / BP017 ΓÇö House Scribe: coordinate-based Jar query. " +
...
server.tool(
  ...
  "KN-J3 / BP017 ΓÇö House Scribe: fire Pixie Dust / Pheromone-write event on a grid cell. " +
  ...
  async (args) => {
    try {
      ...
      if (!result.success) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: result.error, bridle_rule_4: result.bridle_rule_4 }, null, 2) }],
          isError: true,
        };
      }
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ success: true, cell: result.cell, processing_ms: result.processing_ms }, null, 2) }],
      };
    } catch (err) {
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ error: String(err) }, null, 2) }],
 ...
 */
server.tool(
  "house_scribe_query_living_cell",
  "KN-J3 / BP017 ΓÇö House Scribe: query living cell state for a coordinate. " +
 ...
 */
server.tool(
  "house_scribe_gridwork_snapshot",
  "KN-J3 / BP017 ΓÇö House Scribe: full living-gridwork snapshot. " +
    "Returns all tracked cells with living flag + jar_count + density + decay. " +
    "Includes living/dead/fallback cell counts. " +
    "BRIDLE Rule 4: data_available=false on failure; never silent state corruption.",
  {},
  async () => {
    try {
 ...
 */
server.tool(
  "house_scribe_reconcile_cells",
  "KN-J3 / BP017 ΓÇö House Scribe: detect and reconcile cell-state inconsistencies. " +
    "Compares cached cell state against KN-J1 jars ledger (source-of-truth). " +
    "Flags inconsistencies + reconciles. BRIDLE Rule 4 required before any seal operation.",
  {},
  async () => {
    try {
...
server.tool(
  ...
  "KN-J4 / BP017 ΓÇö House Scribe: handle Hive-thread closure event. " +
  ...
  async ({ thread_id, cathedral, cohort_type, closed_at, synthesis_summary, synthesis_blob_pointer, contributors, queen_member_id, content_type, read_cohort_minimum, write_cohort_minimum, total_marks_pool }) => {
    try {
      const event: ThreadClosedWithSynthesisEvent = {
        thread_id,
        cathedral,
        cohort_type,
        closed_at,
 ...
 */
server.tool(
  "house_scribe_apiarist_hive_jar_status",
  "KN-J4 / BP017 ΓÇö House Scribe: query Jars created from a Hive thread. " +
...
server.tool(
  ...
  "KN-J5 / BP017 ΓÇö House Scribe: cross-cathedral Jar query via 8-digit-grid coordinate patterns. " +
    ...
    "BRIDLE Rule 4: insufficient cohort ΓåÆ reject with advancement-suggestion; cathedral unavailable ΓåÆ partial results + flag.",
  {
    pattern: z.string().describe(
      "Cross-cathedral query pattern. Examples: '01-*-*-*' (bishop only), '99-*-*-*' (all cross-cathedral), " +
  ...
  async ({ pattern, querier_cohort_class, querier_cathedral, limit, offset, use_cache }) => {
    try {
      const result = queryCrossCathedral({
        pattern,
        querier_cohort_class: querier_cohort_class as HsCohortClass,
        querier_cathedral,
        limit,
        offset,
 ...
 */
server.tool(
  "house_scribe_invalidate_cross_cache",
  "KN-J5 / BP017 ΓÇö House Scribe: invalidate cross-cathedral query cache. " +
  ...
  async ({ pattern }) => {
    try {
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ success: true, invalidated: pattern ?? "all" }, null, 2) }],
      };
    } catch (err) {
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ error: String(err) }, null, 2) }],
...
server.tool(
  ...
  "KN-J5 / BP017 ΓÇö House Scribe: query the cross-cathedral query provenance log. " +
  ...
  async ({ limit }) => {
    try {
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ data_available: true, entries, count: entries.length }, null, 2) }],
      };
    } catch (err) {
      return {
...
server.tool(
  "on_deck_query",
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: query canonical state file. " +
...
server.tool(
  "on_deck_append",
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: append new K-prompt entry to canonical queue. " +
...
server.tool(
  "on_deck_mutate",
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: mutate entry status. " +
...
server.tool(
  ...
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: attach Shadow E-Giant pre-staging output to a queued entry. " +
...
server.tool(
  ...
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: bulk-import K-prompt files from a dropzone directory. " +
...
server.tool(
  ...
  "KN-Q2 / BP018 ΓÇö On Deck Scribe: aggregated dispatch counts. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: aggregate telemetry counts. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: cost-savings report. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: ordered snapshot timeline for one test_id. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: parallel-session comparison for 5-Knight ├ù N-pod tests. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: anomaly snapshots since cutoff. " +
...
server.tool(
  ...
  "KN-S3 / BP018 ΓÇö Stats-Capture: mark a test_id for indefinite retention. " +
...
server.tool(
  "joules_mint",
  "KN-M3 / BP018 Pod-M ΓÇö Mint a new Forever-Stamp Joule from Marks-surplus. " +
...
server.tool(
  "joules_transfer",
  "KN-M3 / BP018 Pod-M ΓÇö Transfer a Forever-Stamp Joule between members. " +
...
server.tool(
  "joules_redeem",
  "KN-M3 / BP018 Pod-M ΓÇö Redeem a Forever-Stamp Joule against a civilization-class work target. " +
...
server.tool(
  "joules_balance",
  "KN-M3 / BP018 Pod-M ΓÇö Get the current Forever-Stamp Joules balance for a member. " +
...
server.tool(
  "joules_audit",
  "KN-M3 / BP018 Pod-M ΓÇö Aggregate Joules audit: total minted, redeemed, in-circulation, face_value totals. " +
...
server.tool(
  "strata_ascend",
  "KN-T4 / BP018 Pod-T ΓÇö Ascend the 7-layer Keyword-Pyramid from a topic toward Bedrock. " +
...
server.tool(
  "strata_descend",
  "KN-T4 / BP018 Pod-T ΓÇö Descend the 7-layer Keyword-Pyramid from a topic toward Sand. " +
...
server.tool(
  "strata_by_stratum",
  "KN-T4 / BP018 Pod-T ΓÇö List all topics assigned to a given stratum level. " +
...
server.tool(
  "strata_audit",
  "KN-T4 / BP018 Pod-T ΓÇö Audit: count topics per stratum; identify Sand/Soil topics ready for promotion. " +
...
server.tool(
  ...
  "KN-T4 / BP018 Pod-T ΓÇö Recommend Sand/Soil topics for promotion based on pheromone hit-frequency ├ù age. " +
...
server.tool(
  "codex_create",
  "KN-K3 / BP018 Pod-K ΓÇö Create a new Codex in 'drafting' state. " +
...
server.tool(
  "codex_add_chapter",
  "KN-K3 / BP018 Pod-K ΓÇö Add a chapter to a Codex in 'drafting' state. " +
...
server.tool(
  "codex_review",
  "KN-K3 / BP018 Pod-K ΓÇö Move a Codex from 'drafting' to 'review' (mutations frozen for review window). " +
...
server.tool(
  "codex_bind",
  "KN-K3 / BP018 Pod-K ΓÇö Bind a Codex: HMAC-locks all chapters; status ΓåÆ 'bound'; immutable after this. " +
...
server.tool(
  "codex_query",
  "KN-K3 / BP018 Pod-K ΓÇö Query Codices by title, edition, or status. " +
...
server.tool(
  "codex_supersede",
  "KN-K3 / BP018 Pod-K ΓÇö Supersede a bound Codex with a new one. " +
...
server.tool(
  ...
  "KN-K3 / BP018 Pod-K ΓÇö Export a bound Codex to a named Anthology. " +
...
registerTool(
  ...
  "Bushel 32 / BP022 ΓÇö Atomically reserve the next available LB-CODEX-NNNN serial. " +
...
registerTool(
  ...
  "Bushel 32 / BP022 ΓÇö Transition a Codex reservation from status='reserved' to status='bound'. " +
...
registerTool(
  ...
  "Bushel 32 / BP022 ΓÇö Sweep expired reservations (past TTL) and transition them to status='expired', releasing their serials back to the pool. " +
...
registerTool(
  ...
  "Bushel 32 / BP022 ΓÇö Query Codex serial reservations by status, caller, or Bushel number. " +
...
server.tool(
  "gold_tablet_query",
  "KN-N1 / BP018 ΓÇö Gold Tablet: query canonical regulations tablets by tier/scope/topic. " +
...
server.tool(
  "gold_tablet_ratify",
  "KN-N2/N3 / BP018 ΓÇö Gold Tablet: ratify a new canonical regulation. " +
...
server.tool(
  ...
  "KN-N2/N3 / BP018 ΓÇö Gold Tablet: supersede an existing tablet. " +
...
server.tool(
  ...
  "KN-N2/N3 / BP018 ΓÇö Gold Tablet: link an Excalibur Class instance to a Gold Tablet. " +
...
server.tool(
  "gold_tablet_audit",
  "KN-N3 / BP018 ΓÇö Gold Tablet: aggregate audit (counts by tier ├ù status ├ù scope). " +
...
server.tool(
  ...
  "KN-D2 / BP018 ΓÇö Apiarist Hive: create a new hive thread in `open` state. " +
...
server.tool(
  ...
  "KN-D2 / BP018 ΓÇö Apiarist Hive: advance thread state (openΓåÆsynthesizingΓåÆclosedΓåÆsealed). " +
...
server.tool(
  ...
  "KN-D4 / BP018 ΓÇö Apiarist Hive: trigger cross-frame federation on thread close. " +
...
server.tool(
  ...
  "KN-D5 / BP018 ΓÇö Apiarist Hive: enforce 50%-uptime cap per role per cycle. " +
...
server.tool(
  ...
  "KN-J6.2 / BP018 ΓÇö Dual-Tier Addressing: translate local 4-tuple to IPv6 federation address. " +
...
server.tool(
  ...
  "KN-J6.2 / BP018 ΓÇö Dual-Tier Addressing: reverse translate IPv6 federation address to 4-tuple. " +
...
server.tool(
  ...
  "KN-J6.3 / BP018 ΓÇö Dual-Tier Addressing: retrieve provenance chain for a 4-tuple or IPv6 address. " +

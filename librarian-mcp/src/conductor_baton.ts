...
export const TIER_SPECS: Record<SubstrateTier, TierSpec> = {
  haiku: {
...
export interface TierRoutingConfig {
  tiers: {
    haiku: { model: string; cost_per_input_k_usd: number; operations: string[] };
    sonnet: { model: string; cost_per_input_k_usd: number; operations: string[] };
    opus: { model: string; cost_per_input_k_usd: number; operations: string[] };
  };
  routing_rules: {
    fallback_tier: SubstrateTier;
    circuit_breaker_demotion: { haiku_fails_to: SubstrateTier; sonnet_fails_to: SubstrateTier };
  };
}
...
let _cachedConfig: TierRoutingConfig | null = null;

export function loadTierConfig(): TierRoutingConfig {
  if (_cachedConfig) return _cachedConfig;
  const configPath = resolve(__dirname, "..", "conductor", "tier_routing_config.json");
  const raw = readFileSync(configPath, "utf-8");
  _cachedConfig = JSON.parse(raw) as TierRoutingConfig;
 ...
 */
export function classifySubstrateOperation(
  opType: string,
 ...
 */
export function routeSubstrate(
  opType: string,
  options: { circuitBreakerActive?: boolean; config?: TierRoutingConfig } = {},
): TierDecision {
...
export function computeCostRecord(
  ...
  const actualCost =
    (inputTokens / 1000) * spec.cost_per_input_k_usd +
    (outputTokens / 1000) * spec.cost_per_output_k_usd;

  const counterfactualCost =
    (inputTokens / 1000) * sonnetSpec.cost_per_input_k_usd +
 ...
 */
export function computeL2Multiplier(records: SubstrateCostRecord[]): {
 ...
 */
export function computeB127Compound(l2Multiplier: number): {
  l1: number;
  l2: number;
  l3: number;
  l4: number;
...
export function logSubstrateCostRecord(
  record: SubstrateCostRecord,

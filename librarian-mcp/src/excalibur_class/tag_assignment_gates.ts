...
export interface GateThresholds {
  cathedral_effect_min_lift_pp: number;         // default: 30
  furnace_min_verification_score: number;       // default: 0.7
  adversarial_fence_min_pass_rate: number;      // default: 1.0 (all probes must pass)
  federation_vote_quorum: number;               // default: 0.5 (50% of members must vote)
  federation_vote_approval_threshold: number;   // default: 0.6 (60% yes required)
}
...
export interface GateEvaluationResult {
  all_passed: boolean;
  gates: {
    cathedral_effect: { passed: boolean; lift_pp: number; threshold: number; note: string };
    furnace: { passed: boolean; score: number; threshold: number; note: string };
    adversarial_fence: { passed: boolean; pass_rate: number; threshold: number; note: string };
    federation_vote: { passed: boolean; quorum_met: boolean; threshold_met: boolean; note: string };
  };
  bridle_note: string;
 ...
 */
export function evaluateExcaliburGates(
  gates: ExcaliburTagGates,

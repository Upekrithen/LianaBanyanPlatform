...
import type { ExcaliburSubscription, SubscriptionState } from "./types.js";
...
function readAllSubscriptions(): ExcaliburSubscription[] {
  ensureDir();
  if (!existsSync(SUBSCRIPTIONS_PATH)) return [];
  const lines = readFileSync(SUBSCRIPTIONS_PATH, "utf-8").split("\n").filter(l => l.trim());
  const byId = new Map<string, ExcaliburSubscription>();
  for (const line of lines) {
    try {
      const sub = JSON.parse(line) as ExcaliburSubscription;
      byId.set(sub.id, sub);
    } catch {
      continue;
    }
...
function persistSubscription(sub: ExcaliburSubscription): void {
  ensureDir();
...
export function createSubscription(
  subscriberId: string,
  sliceId: string,
  granularity: ExcaliburSubscription["granularity"],
): ExcaliburSubscription {
  const now = new Date().toISOString();
  const sub: ExcaliburSubscription = {
    id: randomUUID(),
...
export function activateSubscription(
  sub: ExcaliburSubscription,
  stripeSubscriptionId?: string,
): ExcaliburSubscription {
  const now = new Date();
  const updated: ExcaliburSubscription = {
    ...sub,
...
export function activateOneTimeAccess(
  sub: ExcaliburSubscription,
...
export function cancelSubscription(sub: ExcaliburSubscription): ExcaliburSubscription {
  const now = new Date().toISOString();
  const updated: ExcaliburSubscription = {
    ...sub,
    state: "cancelled",
...
export function lapseSubscription(sub: ExcaliburSubscription): ExcaliburSubscription {
  const now = new Date().toISOString();
  const updated: ExcaliburSubscription = {
    ...sub,
    state: "lapsed",
...
export function isSubscriptionActive(sub: ExcaliburSubscription): boolean {
  if (sub.state === "active_subscription") return true;
...
export function getActiveSubscriptions(subscriberId: string): ExcaliburSubscription[] {
...
export function hasSliceAccess(subscriberId: string, sliceId: string): boolean {
  return readAllSubscriptions().some(
    s => s.subscriber_id === subscriberId && s.slice_id === sliceId && isSubscriptionActive(s),
  );
}

/** Returns all subscriptions for a slice (for public dashboard). */
export function getSubscriptionsForSlice(sliceId: string): ExcaliburSubscription[] {
  return readAllSubscriptions().filter(s => s.slice_id === sliceId);
}

/** Returns all subscriptions for a subscriber. */
export function getSubscriptionsForSubscriber(subscriberId: string): ExcaliburSubscription[] {

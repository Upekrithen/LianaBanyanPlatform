...
function getLedgerPath(ebletPath: string): string {
...
function appendLedger(ledgerPath: string, record: IronTabletLedgerEntry): void {
 ...
 */
function countLedgerEntriesForPath(ledgerPath: string, ebletPath: string): number {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (!trimmed) continue;
    try {
      const entry = JSON.parse(trimmed) as IronTabletLedgerEntry;
      if (entry.ebletPath === ebletPath) count++;
    } catch {
...
export function readLedgerEntries(ledgerPath: string): IronTabletLedgerEntry[] {
  if (!existsSync(ledgerPath)) return [];
  const raw = readFileSync(ledgerPath, "utf-8");
  const entries: IronTabletLedgerEntry[] = [];
  for (const line of raw.split("\n")) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    try {
      entries.push(JSON.parse(trimmed) as IronTabletLedgerEntry);
    } catch {
...
export async function stoneAppend(params: {
  ...
  return withMutex(ledgerPath, async () => {
    ...
    const entry: IronTabletLedgerEntry = {
      ts,
 ...
 */
export function stoneReadProvenance(ebletPath: string): ProvenanceReceipt[] {

...
export const CATHEDRAL_FROM_ID: Record<string, string> = Object.fromEntries(
  Object.entries(CATHEDRAL_IDS).map(([k, v]) => [v, k])
);
...
export const TIER_FROM_ID: Record<string, string> = Object.fromEntries(
  Object.entries(TIER_IDS).map(([k, v]) => [v, k])
);
...
export const FLAVOR_FROM_ID: Record<string, string> = Object.fromEntries(
  Object.entries(FLAVOR_IDS).map(([k, v]) => [v, k])
);
...
export interface ParsedCoordinate {
  raw: string;
  cathedral_id: string;    // "01"-"99"
  tier_id: string;         // "01"-"99"
  flavor_id: string;       // "01"-"99"
  jar_slot: string;        // "00"-"99"
  cathedral_name: string;  // human-readable
  tier_name: string;
  flavor_name: string;
 ...
 */
export function parseCoordinate(raw: string): ParsedCoordinate | null {
  const parts = raw.split("-");
  if (parts.length !== 4) return null;

  const [cathedral_id, tier_id, flavor_id, jar_slot] = parts;
  if (
    !/^\d{2}$/.test(cathedral_id) ||
    !/^\d{2}$/.test(tier_id) ||
    !/^\d{2}$/.test(flavor_id) ||
    !/^\d{2}$/.test(jar_slot)
  ) return null;
...
export function validateCoordinate(raw: string): ValidationResult {
  const errors: string[] = [];
  ...
  if (!/^\d{2}$/.test(cathedral_id)) errors.push(`Invalid cathedral_id '${cathedral_id}': must be 2 digits`);
  if (!/^\d{2}$/.test(tier_id)) errors.push(`Invalid tier_id '${tier_id}': must be 2 digits`);
  if (!/^\d{2}$/.test(flavor_id)) errors.push(`Invalid flavor_id '${flavor_id}': must be 2 digits`);
  if (!/^\d{2}$/.test(jar_slot)) errors.push(`Invalid jar_slot '${jar_slot}': must be 2 digits`);
  ...
  if ((catNum < 1 || catNum > 99)) errors.push(`Cathedral ID ${cathedral_id} out of range 01-99`);
  if ((tierNum < 1 || tierNum > 99)) errors.push(`Tier ID ${tier_id} out of range 01-99`);
  if ((flavorNum < 1 || flavorNum > 99)) errors.push(`Flavor ID ${flavor_id} out of range 01-99`);
  if (slotNum < 0 || slotNum > 99) errors.push(`Jar slot ${jar_slot} out of range 00-99`);
...
export function cellPrefix(coordinate: string): string {
  const parts = coordinate.split("-");
  return parts.slice(0, 3).join("-");
}

/** Extract jar_slot number from a coordinate. */
export function jarSlot(coordinate: string): number {
  const parts = coordinate.split("-");
 ...
 */
export function parseWildcardQuery(pattern: string): CoordinateQuery | null {
 ...
 */
export function coordinateMatchesQuery(coordinate: string, query: CoordinateQuery): boolean {
  const parsed = parseCoordinate(coordinate);
  if (!parsed) return false;

  if (query.cathedral_id && query.cathedral_id !== "*" && parsed.cathedral_id !== query.cathedral_id) return false;
  if (query.tier_id && query.tier_id !== "*" && parsed.tier_id !== query.tier_id) return false;
  if (query.flavor_id && query.flavor_id !== "*" && parsed.flavor_id !== query.flavor_id) return false;
  if (query.jar_slot && query.jar_slot !== "*" && parsed.jar_slot !== query.jar_slot) return false;
  ...
  if (query.flavor_range) {
    ...
    if (flavorNum < fromNum || flavorNum > toNum) return false;
  }
 ...
 */
export function swarmDaughterCell(overflowCellPrefix: string): string | null {
...
export function cathedralToId(cathedral: string): string {
  return CATHEDRAL_IDS[cathedral] ?? CATHEDRAL_IDS["cross"];
}

export function tierNameToId(tier: string): string {
  return TIER_IDS[tier] ?? TIER_IDS["freeway"]; // default Freeway = Layer 6 (Jars of Honey)
}

export function flavorNameToId(flavor: string): string {
 ...
 */
export function twoDToFourD(tier_id: string, flavor_id: string): string {
 ...
 */
export function fourDToTwoD(coordinate: string): { tier_id: string; flavor_id: string } | null {
  const parsed = parseCoordinate(coordinate);

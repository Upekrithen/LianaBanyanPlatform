...
import type { ConceptsIndex, ConceptEntry } from "../types.js";
...
function extractKeywords(title: string, content: string, tags: string[]): string[] {
  const keywords = new Set<string>(tags.map(t => t.toLowerCase()));
...
function extractRelatedConcepts(content: string): string[] {
  ...
  for (const m of seeAlsoMatches) {
    const ref = m[1].trim();
    if (ref.length > 3 && ref.length < 100) related.add(ref);
  }
  ...
  if (relatedSection) {
    ...
    for (const m of items) {
      ...
      if (item.length > 2 && item.length < 80) related.add(item);
    }
  }
...
export async function parseConcepts(workspaceRoot: string): Promise<ConceptsIndex> {
  ...
  for (const file of allFiles) {
    ...
    if (fm.title?.includes("_index") || file.endsWith("_index.md")) continue;
    ...
    const category = (fm.category as string) || section;
    ...
    for (const kw of keywords) {
      ...
      if (!byKeyword[kw].includes(slug)) byKeyword[kw].push(slug);
    }
  }

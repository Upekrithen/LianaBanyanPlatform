...
import type { TranscriptIndex, TranscriptEntry } from "../types.js";
...
function parseJsonlFile(filePath: string): TranscriptEntry | null {
  try {
      ...
      try {
        ...
        if (event.role === "user" || event.type === "user") {
          ...
          if (typeof text === "string") {
            if (!firstUserMessage && text.length > 10) firstUserMessage = text.slice(0, 300);
        ...
        for (const m of fileMatches) files.add(m[0]);
      } catch {
        continue;
      }
    }
    ...
    if (totalMessages < 2) return null;
...
export async function parseTranscripts(transcriptsDir?: string): Promise<TranscriptIndex> {

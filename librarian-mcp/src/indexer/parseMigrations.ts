...
import type { SchemaIndex, TableSchema, ColumnDef } from "../types.js";
...
function parseColumns(body: string): ColumnDef[] {
  ...
  for (const raw of lines) {
    ...
    if (["CREATE", "ALTER", "DROP", "INSERT", "UPDATE", "DELETE", "ENABLE", "GRANT", "REVOKE", "IF", "ON", "SET", "WITH"].includes(name.toUpperCase())) continue;
...
export async function parseMigrations(workspaceRoot: string): Promise<SchemaIndex> {
  ...
  const featureFlags: Record<string, { value: string; notes?: string }> = {};
  ...
  for (const file of files) {
    ...
    const alterMatches = sql.matchAll(
      /ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(\w+)\s+ADD\s+(?:COLUMN\s+)?(?:IF\s+NOT\s+EXISTS\s+)?(\w+)\s+([\w\[\]()]+)/gi
    );
    ...
    const flagInserts = sql.matchAll(
      /INSERT\s+INTO\s+feature_flags\s*\([^)]*\)\s*VALUES\s*\(([^)]+)\)/gi
    );
    ...
    const flagUpserts = sql.matchAll(
      /INSERT\s+INTO\s+feature_flags\s*\([^)]*\)\s*VALUES\s*\(([^)]+)\)\s*ON\s+CONFLICT/gi
    );

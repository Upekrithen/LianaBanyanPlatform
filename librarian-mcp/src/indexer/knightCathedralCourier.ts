...
function loadTablets(path: string): Record<string, unknown>[] {
...
function buildDedupSet(tablets: Record<string, unknown>[]): Set<string> {
  const keys = new Set<string>();
  for (const t of tablets) {
    const session = (t.source_session as string) ?? "";
    const doc = (t.source_document as string) ?? "";
    if (session || doc) {
...
function appendTablet(path: string, tablet: Record<string, unknown>): void {
...
function extractKNumber(filename: string): string | null {
  const m = filename.match(/PROMPT_KNIGHT_(K\d+[a-z]?)/i);
  return m ? m[1].toUpperCase() : null;
}

/** Extract B-number from a prompt filename. */
function extractBNumber(filename: string): string | null {
...
function inferStatus(filename: string, content: string): string {
  const lower = content.toLowerCase();
  if (lower.includes("## next") || lower.includes("next — dispatch")) return "NEXT";
  return "QUEUED";
}

/** Scan BISHOP_DROPZONE/01_KnightPrompts/ and sync KnightQueue.jsonl. */
function syncKnightQueue(dedupKeys: Set<string>): number {
  const PROMPTS_DIR = resolve(WORKSPACE, "BISHOP_DROPZONE/01_KnightPrompts");
  if (!existsSync(PROMPTS_DIR)) return 0;
  ...
  for (const filename of files) {
    ...
    const tablet: Record<string, unknown> = {
      observation,
...
function syncKnightHandoffs(dedupKeys: Set<string>): number {
  const HANDOFFS_DIR = resolve(WORKSPACE, "BISHOP_DROPZONE/03_BishopHandoffs");
  if (!existsSync(HANDOFFS_DIR)) return 0;

  const files = readdirSync(HANDOFFS_DIR).filter(f =>
    f.startsWith("REPORT_KNIGHT_") && f.endsWith(".md")
  );
  ...
  for (const filename of files) {
    ...
    if (commitMatch) tablet.commit_hash = commitMatch[1];
    if (tagMatch) tablet.git_tag = tagMatch[1];
...
function syncGitTags(dedupKeys: Set<string>): number {
  let tagLines: string[] = [];
  try {
  ...
  for (const tag of tagLines) {
    ...
    if (commitHash) tablet.commit_hash = commitHash;
...
export async function runKnightCathedralCourier(): Promise<CourierResult> {

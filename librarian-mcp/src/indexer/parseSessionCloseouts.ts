...
function extractDate(body: string): string | undefined {
  ...
  if (sessionLine) return sessionLine[1];
...
function extractSummary(body: string): string {
  ...
  candidate = candidate
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (candidate.length > 800) candidate = candidate.slice(0, 797).trimEnd() + "...";
  return candidate || "(no summary extractable)";
}

function extractFilesChanged(body: string): { files: string[]; migrations: string[]; functions: string[]; pages: string[] } {
...
function extractPendingWork(body: string): string[] {
  const out: string[] = [];
...
export async function parseSessionCloseouts(workspaceRoot: string): Promise<ParsedCloseout[]> {
  const pattern = `${workspaceRoot.replace(/\\/g, "/")}/${CLOSEOUT_GLOB}`;
  const files = await glob(pattern, { absolute: true, nodir: true });
  const out: ParsedCloseout[] = [];
  ...
  const bestByBnum = new Map<number, ParsedCloseout>();
  ...
  for (const filePath of files) {
    ...
    if (score > prevScore) {
      ...
      try {
        ...
        if (a > b) bestByBnum.set(bNumber, entry);
      } catch { /* keep prev */ }
    }
  }

  for (const e of bestByBnum.values()) out.push(e);
 ...
 */
export async function parseKnightReports(_workspaceRoot: string): Promise<SessionEntry[]> {
  return [];
}

/** Pick the highest B-number from a list of SessionEntry-like values. */
export function pickHighestBSession(sessions: SessionEntry[]): string | undefined {
 ...
 */
export function mergeSessionStreams(
  legacy: SessionEntry[],
  closeouts: SessionEntry[],
): SessionEntry[] {
  const byId = new Map<string, SessionEntry>();
  for (const s of legacy) byId.set(s.id, s);
  for (const s of closeouts) byId.set(s.id, s); // closeout wins

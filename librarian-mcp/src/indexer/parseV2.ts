...
import type { V2MigrationIndex, V2DomainStatus, DomainIndex } from "../types.js";
...
export async function parseV2(workspaceRoot: string, v1Domains?: DomainIndex): Promise<V2MigrationIndex> {
  const v2Root = resolve(workspaceRoot, "platform-v2");
  const domainsDir = resolve(v2Root, "src", "domains");
  const domains: Record<string, V2DomainStatus> = {};
  ...
  const AUDIT_SESSIONS: Record<string, string> = {
  ...
  for (const domain of V2_DOMAINS) {
    ...
    let auditStatus: V2DomainStatus["auditStatus"] = "not_started";
    if (AUDIT_SESSIONS[domain]) auditStatus = "audited";
    if (v2Pages > 0 || v2Components > 0) auditStatus = "migrated";

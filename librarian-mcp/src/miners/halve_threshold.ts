...
export interface HalveThresholdConfig {
  keyword_density_delta: number;      // threshold for topic-density shift (default 0.3)
  semantic_drift_threshold: number;   // threshold for semantic drift (default 0.4)
  founder_ratification_override?: boolean; // explicit YES/NO from Founder
}
...
export interface HalveDecision {
  ...
  confidence: number;           // 0.0–1.0; below 0.6 = ambiguous → default NO halve
  new_categories: string[];     // topics that triggered the decision
  detection_signals: {
 ...
 */
export function detectHalveThreshold(
  topicsDiscovered: string[],
  config: HalveThresholdConfig,
): HalveDecision {
  ...
  // Confidence score
  const thresholdsMet =
    (keywordDensityDelta >= config.keyword_density_delta ? 0.5 : 0) +

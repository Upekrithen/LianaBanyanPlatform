...
// ─── Miner Corpus Prospecting Input ───────────────────────────────────────

export interface MinerProspectInput {
  claim: string;
  cathedral: string;
  raw_corpus: string[];             // texts to mine (e.g. scribe tablet contents)
  parent_serial?: string;           // set for daughter Miners
  halve_threshold_config?: {
    keyword_density_delta: number;  // min keyword-density change to trigger halve (default 0.3)
    semantic_drift_threshold: number; // min semantic drift score (default 0.4)
    founder_ratification_override?: boolean; // explicit override
  };
  session_id?: string;
}

// ─── Miner Prospecting Result ─────────────────────────────────────────────

export interface MinerProspectResult extends MinerAgentReport {
  role: "miner";
  well_of_knowledge_seeds: string[];       // topic seeds sent to daughter Wells
  knowledge_depth_map: Record<string, number>; // topic -> depth level (1-6)
}
 ...
 */
const KNOWLEDGE_DEPTH_LABELS: Record<number, string> = {
...
function classifyKnowledgeDepth(topicFrequency: number, totalTopics: number): number {
  if (totalTopics === 0) return 6;
...
function extractCorpusTopics(corpus: string[]): Map<string, number> {
  const freqMap = new Map<string, number>();
  const tokenRegex = /\b([a-z][a-z_\-]{3,30})\b/gi;

  const stopWords = new Set(
    "the a an and or of to in for is are was were be been being it this that at by on with as from".split(" ")
  );
 ...
 */
export async function runMinerProspect(input: MinerProspectInput): Promise<MinerProspectResult> {
  const {
    claim,
    cathedral,
    raw_corpus,
  ...
  // Knowledge depth map
  const knowledgeDepthMap: Record<string, number> = {};
  ...
  const halveDecision = detectHalveThreshold(
    topicsDiscovered,
    halve_threshold_config ?? { keyword_density_delta: 0.3, semantic_drift_threshold: 0.4 },
  );

  // Spawn daughter Miners for newly discovered categories (BRIDLE Rule 4: conservative)
  const halvedOffspring: string[] = [];
  const wellOfKnowledgeSeeds: string[] = [];

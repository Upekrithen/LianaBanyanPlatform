...
export interface WellOfKnowledgeEntry {
  daughter_serial: string;     // The Miner serial that owns this Well
  parent_serial: string;       // The Miner that spawned this Well
  topic_seed: string;          // The topic category this Well specializes in
  cathedral: string;           // Cathedral this Well lives in
...
function ensureWellsDir(): void {
  if (!existsSync(WELLS_DIR)) mkdirSync(WELLS_DIR, { recursive: true });
}

function appendWellEntry(entry: WellOfKnowledgeEntry): void {
  ensureWellsDir();
 ...
 */
export function seedWellOfKnowledge(
  daughterSerial: string,
  topicSeed: string,
  parentSerial: string,
  cathedral: string,
): WellOfKnowledgeEntry {
  const entry: WellOfKnowledgeEntry = {
...
export function listWellsForCathedral(cathedral: string): WellOfKnowledgeEntry[] {
  ensureWellsDir();
  if (!existsSync(WELLS_REGISTRY_PATH)) return [];
  const lines = readFileSync(WELLS_REGISTRY_PATH, "utf-8").split("\n").filter(l => l.trim());
  const results: WellOfKnowledgeEntry[] = [];
  for (const line of lines) {
    try {
      const entry = JSON.parse(line) as WellOfKnowledgeEntry;
      if (entry.cathedral === cathedral) results.push(entry);
    } catch {
      continue;
    }
  }
...
export function getWellForSerial(serial: string): WellOfKnowledgeEntry | null {
  ensureWellsDir();
  if (!existsSync(WELLS_REGISTRY_PATH)) return null;
  const lines = readFileSync(WELLS_REGISTRY_PATH, "utf-8").split("\n").filter(l => l.trim());
  for (const line of lines.reverse()) {
    try {
      const entry = JSON.parse(line) as WellOfKnowledgeEntry;
      if (entry.daughter_serial === serial) return entry;
    } catch {
      continue;
    }
  }
...
export function listAllWells(): WellOfKnowledgeEntry[] {
  ensureWellsDir();
  if (!existsSync(WELLS_REGISTRY_PATH)) return [];
  const lines = readFileSync(WELLS_REGISTRY_PATH, "utf-8").split("\n").filter(l => l.trim());
  const results: WellOfKnowledgeEntry[] = [];
  for (const line of lines) {
    try {
      results.push(JSON.parse(line) as WellOfKnowledgeEntry);
    } catch {
      continue;
    }
  }

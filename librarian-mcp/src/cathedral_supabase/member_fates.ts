...
// ─── Clotho — theme extraction (member-scoped) ───────────────────────────
 ...
 */
export function clothoExtractForMember(
  text: string,
  scribes: MemberScribeRow[],
): ClothoOutput {
  const found = new Map<string, string>();
  const entities = new Set<string>();
  if (!text || text.trim().length === 0) return { themes: [], entities: [] };
...
export async function memberFatesRoute(
  input: MemberFatesInput,
): Promise<MemberFatesResult | MemberFatesError> {
  const t0 = Date.now();
  ...
  const scribes = (scribesRes.data ?? []) as MemberScribeRow[];
  ...
  for (const s of scribes) {
    ...
    if (r.score > 0) details.push({ scribe: s, ...r });
  }

  // Atropos
  const dispatchCap = clamp(input.dispatch_cap ?? MAX_DISPATCH_DEFAULT, 1, 10);
  const eligible: MemberFatesDispatch[] = [];
  for (const d of details) {
    ...
    if (!meetsPrimary && !meetsAdjacent) continue;
  ...
  for (const d of details) {
    for (const t of d.primaryMatches) captured.add(t.toLowerCase());
    for (const t of d.adjacentMatches) captured.add(t.toLowerCase());
  }
  const coverage_gaps: string[] = [];
  for (const t of themes) {
    const lo = t.toLowerCase();
    if (captured.has(lo)) continue;
    if (lo.length < 4) continue;
  ...
  if (!triple_witness_met && coverage_gaps.length === 0) {
    coverage_gaps.push(
      `triple-witness threshold not met (${details.length}/${TRIPLE_WITNESS_THRESHOLD} Scribes matched)`,
    );
  }

  // Persist
  const content_hash = sha256(input.content);
  let fates_log_id: string | null = null;
  ...
  if (persist) {
    ...
    fates_log_id = (insertRes.data as { log_id: string }).log_id;
  }

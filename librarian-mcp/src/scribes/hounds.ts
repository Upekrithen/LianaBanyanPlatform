...
export function inboundPheromonePathFor(cathedral: string): string {
...
export interface InboundPheromoneRecord {
  ts: string;                    // ISO-8601 emit timestamp (from source record)
  source_cathedral: string;      // "bishop" | "knight" | "pawn"
  scribe: string;
  tablet_id: string;
  topics_compact: string[];      // top-5 topics by extraction order
  decay_constant_days: number;
  original_index_ref: string;    // composite key: "source_cathedral::scribe::tablet_id"
}
 ...
 */
export function propagatePheromone(
  record: PheromoneRecord,
  sourceCathedral: "bishop" | "knight" | "pawn",
): void {
  const inbound: InboundPheromoneRecord = {
    ts: record.ts,
  ...
  for (const sibling of ALL_CATHEDRALS) {
    if (sibling === sourceCathedral) continue;
    ...
    try {
      ...
      process.stderr.write(
        `[hounds] propagation to ${sibling} failed ` +
...
// ─── Inbound status (for pheromone_inbound_status MCP tool) ───────────────
 ...
 */
export function getInboundStatus(): InboundStatus[] {

 ...
 */
export function getAutoKeywordsDir(): string {
 ...
 */
export function getAutoKeywordsDirForBase(cathedralScribesDir: string): string {
 ...
 */
function resolveKeeperPattern(pattern: string): string[] {
  ...
  for (const base of bases) {
    ...
    if (existsSync(candidate) && statSync(candidate).isFile()) {
      if (TEXT_EXTENSIONS.has(extname(candidate).toLowerCase())) return [candidate];
    }
  }
 ...
 */
export function resolveKeeperPaths(patterns: string[]): string[] {
...
function deepExtractStrings(val: unknown, acc: string[] = []): string[] {
  if (typeof val === "string") { acc.push(val); return acc; }
  if (Array.isArray(val)) { val.forEach((v) => deepExtractStrings(v, acc)); return acc; }
 ...
 */
function shouldDrop(tokens: string[]): boolean {
  // All numeric
  if (tokens.every((t) => /^[\d.,%-]+$/.test(t))) return true;
  // All non-alpha characters (no letter in the entire gram)
  const joined = tokens.join("");
  if (!/[a-z]/.test(joined)) return true;
  // All stopwords
  if (tokens.every((t) => STOPWORDS.has(t))) return true;
  return false;
}
 ...
 */
export function tokenizeToNgrams(text: string, maxN: number = MAX_NGRAM): Map<string, number> {
...
interface ScribeCorpus {
  scribeId: string;
  termFreq: Map<string, number>;  // tf_S(t)
  sourceHash: string;
  fileCount: number;
}
 ...
 */
function buildScribeCorpus(scribeId: string, filePaths: string[]): ScribeCorpus | null {
 ...
 */
function computeDf(corpora: ScribeCorpus[]): Map<string, number> {
...
function selectKeywords(
  ...
  const candidates: Array<{ term: string; score: number; exclusive: boolean }> = [];

  for (const [term, tf] of corpus.termFreq) {
    if (tf < MIN_TF) continue;
    const d = df.get(term) ?? 1;
    if (d > dfCeiling) continue;
 ...
 */
export function extractAutoKeywords(scribeId: string, keeperPaths: string[]): KeywordList {
  ...
  const df = new Map<string, number>();
  for (const [term] of corpus.termFreq) df.set(term, 1);
 ...
 */
export function extractAllAutoKeywords(registry: ScribesRegistry): Map<string, AutoExtractSummary> {
  const corpora: ScribeCorpus[] = [];
  ...
  const results = new Map<string, AutoExtractSummary>();
  for (const corpus of corpora) {
 ...
 */
export function loadAutoKeywordSidecar(scribeId: string, autoKeywordsDir: string): KeywordList {
  const sidecarPath = resolve(autoKeywordsDir, `${scribeId}.yaml`);
  if (!existsSync(sidecarPath)) return [];
  try {
    const raw = readFileSync(sidecarPath, "utf-8");
    const parsed = yaml.load(raw) as Partial<AutoKeywordSidecar> | undefined;
    if (!parsed || !Array.isArray(parsed.keywords)) return [];
    return (parsed.keywords as unknown[]).filter((k) => typeof k === "string") as string[];
  } catch {
 ...
 */
export function writeAutoKeywordSidecar(
  summary: AutoExtractSummary,
  autoKeywordsDir: string,
): void {
  mkdirSync(autoKeywordsDir, { recursive: true });
  const sidecar: AutoKeywordSidecar = {

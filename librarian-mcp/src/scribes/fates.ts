 ...
 */
export function clothoExtract(text: string): { themes: string[]; entities: string[] } {
 ...
 * tighten it, or skip.
 */
export function atroposDispatch(
  themes: string[],
  details: Map<string, { score: number; primaryMatches: string[]; adjacentMatches: string[] }>,
): FatesDispatch[] {
  const eligible: FatesDispatch[] = [];
  const reg = getRegistry();
  const byId = new Map<string, ScribeEntry>(reg.scribes.map((s) => [s.id, s]));
...
export function findCoverageGaps(
  ...
  for (const t of themes) {
    ...
    if (captured.has(lower)) continue;
...
export function runFates(text: string): FatesResult {

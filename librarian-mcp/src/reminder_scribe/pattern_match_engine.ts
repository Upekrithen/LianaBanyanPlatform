...
import {
  ...
  RulesRegistry,
  DEFAULT_PREFERENCES,
  type ReminderScribePreferences,
} from "./rules_registry.js";
 ...
 */
function matchRule(
  rule: ReminderScribeRule,
  text: string,
  workspaceRoot: string,
  prefs: ReminderScribePreferences
): ViolationMatch[] {
 ...
 */
export function runReminderScribeCheck(
  ...
  const prefs: ReminderScribePreferences = {
    ...DEFAULT_PREFERENCES,
    ...opts.preferences,
  };
  ...
  try {
    ...
    return {
      ...
      clean: false,
      summary:
        "BRIDLE Rule 4 HALT: Reminder Scribe engine failed to initialize rules registry. " +
  ...
  for (const rule of sortedRules) {
    ...
    if (confirmedMatches.length === 0) continue;
  ...
  const structurally_immutable_violations = violations.filter(
    (v) => v.override_class === "structurally-immutable"
  ).length;
...
function buildSummary(
  ...
  if (structural.length > 0) {
    parts.push(
      `STRUCTURALLY-IMMUTABLE (cannot override): ${structural.map((v) => v.rule_id).join(", ")}. ` +
  ...
  if (blocking.length > 0 && structural.length === 0) {
    parts.push(
      `BLOCKING violations (Marks-cost to override): ${blocking.map((v) => v.rule_id).join(", ")}. ` +
  ...
  if (reviewable.length > 0) {
    parts.push(
      `Review violations (free to override): ${reviewable.map((v) => v.rule_id).join(", ")}. ` +
...
// ─── Violation event schema (for Detective TEAM write-back — KN-I3) ───────────
 ...
 */
export function buildViolationEvent(
  flag: ViolationFlag,
  opts: {
    session_id: string;
    correction_applied: boolean;
  ...
  const override_marks_cost =
    flag.override_class === "marks-cost" && opts.override_used ? 1 : 0;

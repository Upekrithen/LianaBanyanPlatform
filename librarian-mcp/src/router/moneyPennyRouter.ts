...
import type {
  SchemaIndex, FunctionIndex, PageIndex, CephasIndex, ContextIndex,
  BishopIndex, DomainIndex, ConceptsIndex, DropzoneIndex, TranscriptIndex,
  ComponentIndex, SystemOverview, ArchitecturalRule, SessionEntry,
  BriefingPackage, ChecklistResult, DebriefResult,
} from "../types.js";
import {
  ...
  for (const [domainName, domain] of Object.entries(domains.domains)) {
    ...
    for (const kw of keywords) {
      ...
      for (const p of domain.pages) {
        if (p.toLowerCase().includes(kw)) score += 1;
      }
    }

    if (score > 0) scores[domainName] = score;
  }
...
export function buildBriefing(
  ...
  concepts: ConceptsIndex | null,
  domains: DomainIndex | null,
  context: ContextIndex | null,
  dropzones: DropzoneIndex | null,
  ...
  // 2. Concept matching
  const relevantConcepts: BriefingPackage["relevantConcepts"] = [];
  if (concepts) {
    const matchedSlugs = new Set<string>();
    for (const kw of keywords) {
      const byKw = concepts.byKeyword[kw];
      if (byKw) {
        for (const slug of byKw.slice(0, 3)) matchedSlugs.add(slug);
      }
    }
  ...
  // 3. Applicable rules
  const applicableRules: BriefingPackage["applicableRules"] = [];
  ...
  // 4. Past work
  const pastWork: BriefingPackage["pastWork"] = [];
  if (context) {
...
export function buildChecklist(
  ...
  // Consistency check
  const violations: ChecklistResult["violations"] = [];
  const warnings: ChecklistResult["warnings"] = [];
  ...
  // Related sessions
  const relatedSessions: ChecklistResult["relatedSessions"] = [];
  if (context) {
...
export function buildDebrief(
  ...
  if (lower.includes("83%") && !lower.includes("83.3%")) consistencyCheck = "WARNING: Use 83.3%, not 83%";
  if (/\b(equity|shares|dividend)\b/.test(lower)) consistencyCheck = "WARNING: SEC-unsafe language in summary";

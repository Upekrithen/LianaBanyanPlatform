...
const CALL_LOG_PATH = path.join(
  __dirname, "..", "stitchpunks", "wrasse", "phase_f_call_log.jsonl"
);
...
let _wrasseMode: "on" | "off" = "off";
let _enabled = false;

export function setPhaseFSession(sessionId: string, wrasseMode: "on" | "off"): void {
  _sessionId = sessionId;
  _wrasseMode = wrasseMode;
  _enabled = true;
}

export function disablePhaseF(): void {
  _enabled = false;
}
...
// ─── MCP response content extractor ─────────────────────────────────────────

type McpContent = { type: string; text?: string };
type McpResult = { content?: McpContent[] };

function extractResponseChars(result: McpResult): number {
...
function appendCallLog(record: Record<string, unknown>): void {
  try {
 ...
 */
export function logToolResponse(toolName: string, result: McpResult): void {
 ...
 */
export function wrapWithPhaseF<T extends McpResult>(
  toolName: string,

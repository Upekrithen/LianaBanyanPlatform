...
export function extractTriggers(claim: string): TriggerCandidate[] {
  const found: TriggerCandidate[] = [];
  const seen = new Set<string>();

  const add = (p: string, c: TriggerClass) => {
  ...
  for (const m of claim.matchAll(K_PREFIX_RX)) add(m[0], "k_prefix");
  for (const m of claim.matchAll(TS_PREFIX_RX)) add(m[0], "ts_prefix");
  for (const m of claim.matchAll(CALL_SIGN_RX)) add(m[0], "call_sign");
...
function loadExistingPatterns(): Map<string, string> {
  const patterns = new Map<string, string>(); // lower(pattern) -> trigger_id
  if (!fs.existsSync(REGISTRY_PATH)) return patterns;
  const lines = fs.readFileSync(REGISTRY_PATH, "utf8").split("\n");
  for (const raw of lines) {
    const line = raw.trim();
    if (!line) continue;
    try {
      const obj: RegistryEntry = JSON.parse(line);
      if (obj.record_type === "supersedes") continue;
...
function nextTriggerId(): string {
  let maxN = 0;
  if (!fs.existsSync(REGISTRY_PATH)) return "W-001";
  const lines = fs.readFileSync(REGISTRY_PATH, "utf8").split("\n");
  for (const raw of lines) {
    const line = raw.trim();
    if (!line) continue;
    try {
      const obj = JSON.parse(line) as { trigger_id?: string };
...
function buildTriggerRegex(pattern: string, triggerClass: TriggerClass): string {
...
function tryAcquireLock(timeout_ms = 3000): boolean {
...
export function appendIfNew(
  ...
  if (!isEnabled()) return { action: "unchanged", trigger_id: "" };
  ...
  try {
    ...
    if (existing.has(key)) {
      ...
      if (!locked) return { action: "unchanged", trigger_id: existingTid };
      try {
        appendLine({
    ...
    // Novel trigger
    const locked = tryAcquireLock();
    if (!locked) return { action: "unchanged", trigger_id: "" };
    try {
 ...
 */
export function autoRegisterFromDetective(
  claim: string,

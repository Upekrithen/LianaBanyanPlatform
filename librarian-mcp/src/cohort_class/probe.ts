...
import {
  ...
  type CueCardVestingContext,
  type VestingStateResult,
} from "./cue_card_recency.js";
 ...
 */
export async function probeCohortClass(memberId: string): Promise<CohortClassResult> {
  const probed_at = new Date().toISOString();
  const brittle_fallback: CohortClassResult = {
  ...
  if (!client) return brittle_fallback;

  let raw: Record<string, unknown>;
  try {
    // Cast through unknown to avoid strict Supabase generic typing on unknown DB schema
    const rpc = (client.rpc as unknown) as (fn: string, params: Record<string, unknown>) => Promise<{ data: unknown; error: unknown }>;
  ...
  const cohort_class = (raw.cohort_class as CohortClass) ?? "lone_wolf";
  const librarian_mode: LibrarianMode = raw.librarian_mode === "fluid" ? "fluid" : "brittle";
  const membership_status = typeof raw.membership_status === "string" ? raw.membership_status : null;

  // Parse cue card vesting if present
  let cue_card_vesting: VestingStateResult | null = null;
 ...
 */
export function formatCohortSummary(result: CohortClassResult): string {
 ...
 */
export function isFluidMode(mode: LibrarianMode): boolean {
  return mode === "fluid";
}

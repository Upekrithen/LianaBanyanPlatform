...
export type CueCardVestingState =
  | "inactive"          // No qualifying Cue Card sent
  | "active"            // All 3 gates, within last 7 days
  | "expiring_warning"  // Within 24h of expiry; re-up reminder fires
  | "expired";          // >7 days since last qualifying Cue Card; auto-downgrade
 ...
 */
export function calculateVestingState(ctx: CueCardVestingContext, nowMs?: number): VestingStateResult {
 ...
 */
export function cueCardToCohorTier(
  vestingState: CueCardVestingState,
  activeCueCardCount: number,
): "lone_wolf" | "pied_piper_tier_1" | "pied_piper_tier_2_plus" {
 ...
 */
export function buildReUpReminderMessage(state: VestingStateResult): string | null {
  switch (state.state) {
 ...
 */
export function validateVestingContext(raw: unknown): CueCardVestingContext | null {
  if (!raw || typeof raw !== "object") return null;
  const r = raw as Record<string, unknown>;
  if (typeof r.member_id !== "string") return null;
  if (typeof r.active_cue_card_count !== "number") return null;
  return {
    member_id: r.member_id as string,
    active_cue_card_count: r.active_cue_card_count as number,

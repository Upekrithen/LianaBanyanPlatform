 ...
 */
export async function probeTierConfig(memberId: string): Promise<TierConfigResult> {
  const probed_at = new Date().toISOString();
  const notChosenFallback: TierConfigResult = {
    user_id: memberId,
    tier: null,
    tier_chosen_at: null,
  ...
  if (!client) return notChosenFallback;
  ...
  try {
    ...
    return {
      user_id: memberId,
      tier: (raw.tier as ResourceConfigTier) ?? null,
      tier_chosen_at: typeof raw.tier_chosen_at === "string" ? raw.tier_chosen_at : null,
      tier_state: (raw.tier_state as "chosen" | "not_chosen") ?? "not_chosen",
      tier_label: typeof raw.tier_label === "string" ? raw.tier_label : null,
      tier_metadata: raw.tier_metadata as TierMetadata | null ?? null,
      fallback: false,
      probed_at,
    };
  } catch (err) {
 ...
 */
export async function setTierConfig(
  userId: string,
  tier: ResourceConfigTier
): Promise<TierSetResult> {
  const client = getSupabaseClient();
  if (!client) {
    return {
      success: false,
  ...
  try {
    ...
    return {
      ...
      user_id: userId,
      tier,
      prev_tier: (raw.prev_tier as ResourceConfigTier) ?? null,
      tier_chosen_at: typeof raw.tier_chosen_at === "string" ? raw.tier_chosen_at : null,
...
export const TIER_DISPLAY: Record<ResourceConfigTier, { short: string; long: string; tagline: string }> = {
  needs: {
    short: "Tier A — NEEDS",
 ...
 */
export function formatTierSummary(result: TierConfigResult): string {
 ...
 */
export function buildPlanTierAdvisory(tier: ResourceConfigTier, detectedSurface?: string): string | null {
  if (tier !== "founder") return null;
  return (
    `Advisory (informational — does NOT block): You selected Tier C FOUNDER. ` +

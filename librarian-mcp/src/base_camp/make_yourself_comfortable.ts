...
import {
  ...
  DEFAULT_SCOPE,
  ALL_CATEGORIES,
  type UserChoiceScope,
  type IntegrationCategory,
} from "./user_choice_integration.js";
...
interface Registry {
  default_integration_set: Record<string, PathEntry>;
}

function loadRegistry(): Registry {
  try {
    const raw = readFileSync(REGISTRY_PATH, "utf-8");
    return jsYaml.load(raw) as Registry;
  } catch {
    return { default_integration_set: {} };
  }
}

// ─── Shadow sub-task definitions ──────────────────────────────────────────
...
export const SHADOW_TASKS: ShadowTask[] = [
...
export async function runMakeYourselfComfortable(
  options: MakeComfortableOptions = {}
) {
  const {
    scope = "default",
    customScope,
  ...
  // Resolve user scope
  const userScope: UserChoiceScope =
    scope === "custom" && customScope
      ? customScope
      : scope === "minimal"
  ...
  const shadowPathMap = new Map<ShadowId, { patterns: string[]; decayDays: number }>();

  for (const shadow of SHADOW_TASKS) {
    const patterns: string[] = [];
    let decayDays = 60;
    for (const cat of shadow.categories) {
      if (!userScope.enabled_categories.includes(cat)) continue;
      const entry = registry.default_integration_set[cat];
      if (!entry) continue;
      patterns.push(...entry.paths);
      if (entry.decay_constant_days) decayDays = entry.decay_constant_days;
    }
    // Resolve relative paths to workspace root
    const resolved = patterns.map((p) =>
      p.startsWith("/") || p.match(/^[A-Z]:/i)
        ? p
  ...
  // Execute shadows
  const shadowEntries = Array.from(shadowPathMap.entries()).filter(
    ([, v]) => v.patterns.length > 0
  );
  ...
  if (sequential) {
    ...
    shadowResults = await Promise.all(
      shadowEntries.map(([shadowId, { patterns, decayDays }]) =>
        bulkLoadPaths(patterns, {
          shadowId,
          decayConstantDays: decayDays,
          dryRun,
        })
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const scopeArg = process.argv.find((a) => a.startsWith("--scope="))?.split("=")[1] as
    | "default"
    | "minimal"
    | undefined;
  ...
  console.log(
    `Phase-0 hit ratio:   ${receipt.detective_phase0_hit_ratio_pre_load.toFixed(2)} → ${receipt.detective_phase0_hit_ratio_post_load.toFixed(2)} (${receipt.hit_ratio_improvement_factor}×)`
  );

...
// ResourceConfigTier mirrors platform/src/components/handshake/TierSelectionStep.tsx.
// Inlined here to avoid cross-package import (librarian-mcp + platform are separate build roots).
export type ResourceConfigTier = "needs" | "suggests" | "founder";

// ─── Bounty Classes ────────────────────────────────────────────────────────────

export type TierBountyClass =
  | "tier_a_floor_verification"      // Tier A NEEDS empirical floor verification
  | "tier_b_uplift_verification"     // Tier B SUGGESTS uplift verification
  | "tier_c_founder_replication"     // Tier C FOUNDER empirical-receipt-source replication
  | "cross_tier_comparison";         // Cross-tier comparison receipt
...
export interface BountyPayRateMeta {
  ...
  marks_multiplier: number;             // Multiplier against base Bounty Marks rate
  uplift_class: "baseline" | "recommended" | "maximum" | "cross_tier";
  description: string;
  poster_headline: string;
  poster_subhead: string;
  empirical_anchor: string;             // Key empirical doc / receipt pointer
  notes: string;
}

export const TIER_BOUNTY_PAY_RATES: Record<TierBountyClass, BountyPayRateMeta> = {
  tier_a_floor_verification: {
    ...
    description:
      "Verify that Cathedral Effect retrieval lift holds at Tier A (default Claude Code plan). " +
  ...
  tier_b_uplift_verification: {
    ...
    description:
      "Verify the documented uplift of Tier B (Claude Max or equivalent) over Tier A baseline. " +
    ...
    notes:
      "1.25× multiplier for recommended-tier uplift verification. " +
  ...
  tier_c_founder_replication: {
    ...
    description:
      "Replicate Founder's empirical-receipt-source results at Tier C. " +
    ...
    notes:
      "1.5× multiplier + Project-cohort uplift bonus. " +
  ...
  cross_tier_comparison: {
    ...
    description:
      "Submit a controlled cross-tier comparison receipt: identical question bank, " +
    ...
    notes:
      "2.0× multiplier — highest Bounty class. " +
 ...
 */
export function getBountyPayRateByClass(bountyClass: TierBountyClass): BountyPayRateMeta {
  const meta = TIER_BOUNTY_PAY_RATES[bountyClass];
  if (!meta) {
    throw new Error(
      `get_lb_frame_tier_bounty_pay_rate: unknown bounty_class "${bountyClass}". ` +
 ...
 */
export function getPrimaryBountyPayRateForTier(tier: ResourceConfigTier): BountyPayRateMeta {
  const tierToClass: Record<ResourceConfigTier, TierBountyClass> = {
 ...
 */
export function getAllTierBountyPayRates(): BountyPayRateMeta[] {
  return [
...
// ─── MCP tool handler shape (consumed by server.ts registerTool wrapper) ─────
// This export exposes the pure function that server.ts's registerTool wraps.
 ...
 */
export function handleGetTierBountyPayRate(
  args: GetTierBountyPayRateArgs
): GetTierBountyPayRateResult {
  try {
    if (args.list_all) {
    ...
    return {
      ok: false,
      error:
        "Provide one of: bounty_class, tier, or list_all=true. " +

...
} from "./bounty_poster_tier_generator.js";
...
function isValidQuestionBank(value: unknown): boolean {
  if (typeof value !== "string") return false;
  return /^R(10|11|13)/i.test(value as string);
}
 ...
 */
function checkSuspiciousInflation(
  lift_pp: number,
  field_label: string,
  warnings: WarningDetail[],
): void {
  if (lift_pp > SUSPICIOUS_INFLATION_THRESHOLD_PP) {
    warnings.push({
      ...
      message:
        `${field_label} = ${lift_pp}pp exceeds the Cathedral Effect cross-vendor typical ceiling ` +
...
function validateTierA(
  receipt: Record<string, unknown>,
  failures: FailureDetail[],
  warnings: WarningDetail[],
): { margin: number; bridle_rule_4_applied: boolean } {
...
function validateTierB(
  ...
  if (isNaN(velocity)) {
    failures.push({
      ...
      criterion:
        "reckoning_velocity_ratio must be a number (Reckoning velocity vs Tier A baseline)",
    ...
    failures.push({
      field: "reckoning_velocity_ratio",
      criterion:
        `Reckoning velocity at Tier B must be ≥ ${TIER_B_VELOCITY_PASS_THRESHOLD}× Tier A. ` +
    ...
    failures.push({
      field: "reckoning_velocity_ratio",
      criterion:
        `Reckoning velocity ${velocity}× is below the ${TIER_B_VELOCITY_PASS_THRESHOLD}× canonical threshold. ` +
    ...
    warnings.push({
      ...
      message:
        `Reckoning velocity ${velocity}× is borderline (threshold: ${TIER_B_VELOCITY_PASS_THRESHOLD}×). ` +
  ...
  if (!ref || (typeof ref === "string" && ref.trim().length === 0)) {
    failures.push({
      ...
      criterion:
        "tier_a_reference_receipt is required — cannot validate Tier B uplift without Tier A floor anchor",
...
function validateTierC(
  ...
  const lift_pp =
    typeof receipt.replication_lift_pp === "number" ? receipt.replication_lift_pp : NaN;
  const margin = isNaN(lift_pp) ? -CATHEDRAL_LIFT_PASS_PP : lift_pp - CATHEDRAL_LIFT_PASS_PP;
  ...
  if (isNaN(lift_pp)) {
    ...
    failures.push({
      field: "replication_lift_pp",
      criterion:
        `Cathedral Effect lift at Tier C replication must be ≥ ${CATHEDRAL_LIFT_PASS_PP}pp`,
      actual: lift_pp,
      expected: `≥ ${CATHEDRAL_LIFT_PASS_PP}`,
      bridle_rule: "rule_4",
    });
  ...
  if (!ref || (typeof ref === "string" && ref.trim().length === 0)) {
    failures.push({
      ...
      criterion:
        "founder_cascade_reference must reference a valid Tier C receipt anchor " +
  ...
  if (!corpus || (typeof corpus === "string" && corpus.trim().length === 0)) {
    failures.push({
      ...
      criterion:
        "corpus_folder_description is required — confirms submitter used own corpus, " +
  ...
  if (typeof cj === "number" && cj > 30) {
    ...
    warnings.push({
      ...
      message:
        `crown_jewels_surfaced = ${cj}. Founder reference for BP015→BP017 cascade: 15 Crown Jewels. ` +
...
function validateCrossTier(
  ...
  if (receipt.same_submitter !== true) {
    failures.push({
      ...
      criterion:
        "same_submitter must be true — all three tier runs must be by the same member " +
  ...
  if (!isValidQuestionBank(qbv)) {
    failures.push({
      field: "question_bank_version",
      criterion:
        "question_bank_version must be a valid benchmark (R10, R11, R13) and must be " +
  ...
  // Per-tier lift_pp ≥ 30
  const tierFields: Array<{ cold: string; hot: string; label: string }> = [
  ...
  for (const { cold, hot, label } of tierFields) {
    const coldVal = typeof receipt[cold] === "number" ? (receipt[cold] as number) : NaN;
    const hotVal = typeof receipt[hot] === "number" ? (receipt[hot] as number) : NaN;
  ...
  if (!isNaN(hotA) && !isNaN(hotB) && !isNaN(hotC)) {
    if (!(hotC > hotB && hotB > hotA)) {
      warnings.push({
        ...
        message:
          `Non-monotone HOT accuracy pattern: Tier A = ${hotA}%, Tier B = ${hotB}%, Tier C = ${hotC}%. ` +
  ...
  // run_timestamps
  const timestamps = receipt.run_timestamps as Record<string, unknown> | undefined;
  if (!timestamps || typeof timestamps !== "object") {
    failures.push({
      field: "run_timestamps",
      criterion:
        "run_timestamps must be an object with ISO 8601 timestamps for each tier: " +
        "{tier_a: string, tier_b: string, tier_c: string}",
      expected: "object with tier_a, tier_b, tier_c ISO 8601 strings",
    });
  } else {
    for (const key of ["tier_a", "tier_b", "tier_c"] as const) {
 ...
 */
export function validateBountyReceipt(
  bounty_id: string,
  bounty_class: TierBountyClass,
  receipt: Record<string, unknown>,
): BountyReceiptValidationResult {
 ...
 */
export function handleValidateBountyReceipt(
  args: ValidateBountyReceiptArgs,
): ValidateBountyReceiptResult {
  try {
    if (!args.bounty_id) {
    ...
    if (!args.bounty_class) {
      return {
        ok: false,
        error:
          "bounty_class is required. Valid values: " +
    ...
    const validation = validateBountyReceipt(
      args.bounty_id,
      args.bounty_class,
      args.receipt_json,
    );
...
export const KN_H7_TEST_CASES = [
  {
    ...
    args: {
      ...
      bounty_class: "tier_a_floor_verification" as TierBountyClass,
      receipt_json: {
  ...
  },
  {
    ...
    args: {
      ...
      bounty_class: "tier_b_uplift_verification" as TierBountyClass,
      receipt_json: {
  ...
  },
  {
    ...
    args: {
      ...
      bounty_class: "tier_c_founder_replication" as TierBountyClass,
      receipt_json: {
        ...
        corpus_folder_description:
          "Own project folder: 65GB ML research corpus, 12,000 files, 2018-2026.",
  ...
  },
  {
    ...
    args: {
      ...
      bounty_class: "cross_tier_comparison" as TierBountyClass,
      receipt_json: {
  ...
  },
  {
    id: "T7",
    description:
      "BRIDLE Rule 4: borderline velocity (1.7×, below 2×) defaults to FAIL — anti-marketing-class",
    args: {

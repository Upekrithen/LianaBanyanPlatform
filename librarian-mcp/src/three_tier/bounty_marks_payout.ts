...
import { getCathedralClient, getCathedralClientError } from "../cathedral_supabase/client.js";
...
const TIER_MULTIPLIERS: Record<TierBountyClass, number> = {
 ...
 */
export function computeCompletionQualityFactor(margin: number): number {
  if (margin >= 10)  return 1.00;
  if (margin >= 5)   return 0.90;
  if (margin >= 1)   return 0.80;
  if (margin >= 0.5) return 0.75;
 ...
 */
export function computeMarksEarned(
  ...
  if (multiplier === undefined) {
    throw new Error(
      `computeMarksEarned: unknown tier_class "${tier_class}". ` +
...
export interface BountyPayoutLedgerEntry {
  ...
  payout_at:                  string;
  fork_compliant:             true;  // Literal true — FORK doctrine structural attestation
  membership_orthogonal:      true;  // Literal true — $5/year is access-gate only
 ...
 */
export function computeBountyPayout(
  validation: BountyReceiptValidationResult,
  standard_rate: number = 100,
): {
  tier_multiplier: number;
  completion_quality_factor: number;
  marks_earned: number;
  ...
  if (!validation.pass) {
    throw new Error(
      `computeBountyPayout: receipt ${validation.bounty_id} FAILED validation. ` +
 ...
 * cash_out_bounty_marks_to_fiat DOES NOT EXIST in this codebase.
 */
export async function handleProcessBountyMarksPayout(
  args: ProcessBountyMarksPayoutArgs,
): Promise<ProcessBountyMarksPayoutResult> {
  // ── Input validation ────────────────────────────────────────────────────────
  if (!args.receipt_id) {
  ...
  if (!supabase) {
    return {
      ok: false,
      error:
        "Supabase client not configured. Set LIBRARIAN_SUPABASE_URL + " +
  ...
  try {
    ...
    const payout: BountyPayoutLedgerEntry = {
    ...
    return {
      ok: false,
      error: `process_bounty_marks_payout exception: ${(err as Error).message}`,
    };
  }
}
...
export const KN_H8_COMPUTATION_TEST_CASES = [
  {
    ...
    cases: [
      { tier_class: "tier_a_floor_verification"  as TierBountyClass, expected: 1.00 },
      { tier_class: "tier_b_uplift_verification" as TierBountyClass, expected: 1.25 },
      { tier_class: "tier_c_founder_replication" as TierBountyClass, expected: 1.50 },
      { tier_class: "cross_tier_comparison"      as TierBountyClass, expected: 2.00 },
    ],
    verify: (tc: { tier_class: TierBountyClass; expected: number }) => {
  ...
  },
  {
    ...
    verify: (tc: { margin: number; expected: number }) => {
  ...
  },
  {
    ...
    cases: [
      ...
      { tier_class: "tier_a_floor_verification"  as TierBountyClass, standard_rate: 100, margin: 15, expected: 100 },
      // Tier B, margin=7 (quality=0.9): floor(100 × 1.25 × 0.9) = floor(112.5) = 112
      { tier_class: "tier_b_uplift_verification" as TierBountyClass, standard_rate: 100, margin: 7,  expected: 112 },
      // Tier C, margin=3 (quality=0.8): floor(100 × 1.5 × 0.8) = floor(120) = 120
      { tier_class: "tier_c_founder_replication" as TierBountyClass, standard_rate: 100, margin: 3,  expected: 120 },
      // Cross-tier, bare pass margin=0.2 (quality=0.7): floor(100 × 2.0 × 0.7) = 140
      { tier_class: "cross_tier_comparison"      as TierBountyClass, standard_rate: 100, margin: 0.2, expected: 140 },
    ],
    verify: (tc: { tier_class: TierBountyClass; standard_rate: number; margin: number; expected: number }) => {
  ...
  },
  {
    ...
    verify: () => {
      const mockFail: BountyReceiptValidationResult = {
        pass: false,
        margin: -5,
  ...
  },
  {
    ...
    verify: () => {
      ...
      const mockPass: BountyReceiptValidationResult = {
        pass: true,
        margin: 15,
        failures: [],
        warnings: [],

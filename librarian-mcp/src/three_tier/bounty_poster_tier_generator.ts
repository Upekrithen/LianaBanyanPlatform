...
import {
  ...
  type BountyPayRateMeta,
  getBountyPayRateByClass,
  getAllTierBountyPayRates,
} from "./bounty_poster_tier_scaffold.js";

// ─── FORK Doctrine: Marks-class branded type ──────────────────────────────────
// Ensures marks_pay_rate can never be confused with a fiat amount.
// MarksPayRate.currency === "Marks" is structurally enforced at the type level.
...
// ─── Cohort Class Eligibility ─────────────────────────────────────────────────

export type CohortClassEligibility =
  | "federation_member_or_higher"
  | "any_authenticated_user";

// ─── Tier Bounty Poster Object ────────────────────────────────────────────────
...
const SUBMISSION_SCHEMAS: Record<TierBountyClass, SubmissionSchema> = {
  ...
  tier_b_uplift_verification: {
    ...
    required_fields: [
      ...
      },
      {
        ...
        type: "number",
        description:
          "Reckoning velocity ratio vs Tier A (e.g. 2.1 = 2.1× faster Reckoning at Tier B)",
      },
      {
        name: "run_timestamp",
        type: "string",
    ...
    required_fields: [
      {
        ...
        type: "string",
        description:
          "Reference to Founder's BP015→BP017 cascade receipt (canonical anchor: KN-H4 LANDED tier-spec docs)",
      },
      {
      ...
      },
      {
        ...
        type: "number",
        description:
          "Time in hours for Tier C Reckoning run (Founder reference: 36 hours for BP015→BP017 cascade)",
      },
      {
        name: "tier_config",
        type: "string",
      ...
      },
      {
        ...
        type: "string",
        description:
          "Brief description of own folder/project used in replication (does not need to be Founder's corpus)",
      },
      {
        name: "run_timestamp",
        type: "string",
  ...
  cross_tier_comparison: {
    ...
    required_fields: [
      ...
      },
      {
        ...
        type: "string",
        description:
          "R10/R11/R13 question bank version — MUST be same bank across all three tiers for valid comparison",
      },
      {
        name: "tier_a_model",
        type: "string",
      ...
      },
      {
        ...
        type: "object",
        description:
          "ISO 8601 timestamps for each tier run: {tier_a: string, tier_b: string, tier_c: string}",
      },
    ],
    optional_fields: [
      {
...
const VALIDATION_CRITERIA: Record<TierBountyClass, ValidationCriteria> = {
  tier_a_floor_verification: {
    pass_conditions: [
    ...
    validator_notes:
      "KN-H7 validator: check lift_pp ≥ 30, tier_config === 'needs', valid question_bank_version. " +
  ...
  tier_b_uplift_verification: {
    ...
    furnace_score_required: false,
    validator_notes:
      "KN-H7 validator: verify Tier A reference receipt resolves; check velocity_ratio ≥ 1.5. " +
  ...
  tier_c_founder_replication: {
    ...
    furnace_score_required: false,
    validator_notes:
      "KN-H7 validator: 1.5× multiplier includes Apiarist Project-cohort uplift (BP016). " +
  ...
  cross_tier_comparison: {
    ...
    furnace_score_required: false,
    validator_notes:
      "KN-H7 validator: 2.0× multiplier — highest Bounty class. Strict same-submitter, same-bank requirement. " +
 ...
 */
export function generateTierBountyPoster(
  tier_class: TierBountyClass,
  standard_rate: number = 100,
): TierBountyPoster {
  const payRateMeta: BountyPayRateMeta = getBountyPayRateByClass(tier_class);

  // FORK doctrine compliance: currency is locked to "Marks" at construction time.
  const marks_pay_rate: MarksPayRate = {
    currency: "Marks",
 ...
 */
export function generateAllTierBountyPosters(
  standard_rate: number = 100,
): TierBountyPoster[] {
  return getAllTierBountyPayRates().map((meta) =>
    generateTierBountyPoster(meta.bounty_class, standard_rate),
  );
}
 ...
 */
export function handleGenerateTierBountyPoster(
  args: GenerateTierBountyPosterArgs,
): GenerateTierBountyPosterResult {
  try {
    ...
    if (!args.tier_class) {
      return {
        ok: false,
        error:
          "Provide tier_class (tier_a_floor_verification | tier_b_uplift_verification | " +

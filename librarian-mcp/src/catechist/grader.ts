...
export const VIOLATION_LOG_PATH = resolve(
  __dirname_catechist,
  "../../stitchpunks/reminder_scribe/violation_log.jsonl"
);

// ─── R01-R10 base rules (Catechist KN036 canon) ───────────────────────────────
 ...
 */
export const R01_R10_RULES: Omit<RuleGrade, "verdict" | "evidence">[] = [
  {
    rule_id: "R01",
...
// ─── KN-I2: Violation-history-summary extension ───────────────────────────────
 ...
 */
export function buildViolationHistorySummary(
  ai_member: string,
  _opts: { workspaceRoot?: string } = {}
  ...
  try {
    ...
    const eventsByRule: Record<string, Array<{
      corrected: boolean;
      session: string;
      ts: string;
    }>> = {};
    ...
    for (const line of lines) {
      try {
        ...
        if (ts < cutoff) continue; // outside rolling 7-day window

        const ruleId = typeof event.rule_id === "string" ? event.rule_id : null;
        if (!ruleId) continue;

        if (!eventsByRule[ruleId]) eventsByRule[ruleId] = [];
    ...
    const entries: ViolationHistoryEntry[] = ALL_RULE_IDS.map((ruleId) => {
 ...
 */
export function formatGradeMarkdown(result: ExtendedGradeResult): string {
  const lines: string[] = [];

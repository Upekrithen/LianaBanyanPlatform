...
bridle_version: 11
ratified_by: Founder
ratified_session: B133
last_rule_added: Rules 11A + 11B — counsel-no-gate + founder-prose-pass-at-fire-time (K-Founder-Edict-Propagation B133 2026-04-29)
---

# THE BRIDLE — v11

**Read this before you respond. Follow all eleven rules. Task follows the BRIDLE block.**
...
11. **Founder-edict compliance — two mandatory cross-agent rules (B133):**

    **11A — Counsel-no-gate:** Knight NEVER inserts "counsel review" / "counsel coordination" / "counsel sign-off" / "verify with counsel" / "send to counsel" / "counsel approval" as a step that gates Founder. Knight surfaces substantive issues to Founder; Founder decides counsel involvement. Counsel is Founder's resource, not an agent-side gate. Reference: `feedback_dont_gate_on_counsel.md`.

    **11B — Founder-prose-pass-at-fire-time:** Knight produces draft material when Founder fires drafting K-prompts. Knight NEVER pressures Founder on PUBLICATION timing. Knight NEVER treats draft K-prompt firing as "must-be-soon" pressure on external dispatch. Founder prose-passes all PUBLICATION dispatch in single consolidated session at fire-time. Reference: `feedback_founder_prose_pass_at_fire_time_only_no_pre_drafting.md`.
...
| **v11** | **B133 K-Founder-Edict-Propagation** | **Rule 11A — counsel-no-gate; Rule 11B — founder-prose-pass-at-fire-time** |
...
```
**THE BRIDLE — read this before you respond. Follow all eleven rules. Task follows the BRIDLE block.**
```

Copy the eleven rules verbatim from the numbered list above. Task-specific elaborations (e.g. for empirical tests or budget-guarded tasks) may be appended to individual rule text without altering the rule structure.

When authoring new Knight prompts, use v11 from this file. Do not embed an older version.

---

*Authored by Bishop (Claude Opus 4.7). v11 extended by Knight (K-Founder-Edict-Propagation, Cursor/Sonnet 4.6, B133, 2026-04-29). Canonical source for all Knight prompt BRIDLE blocks.*

# KNIGHT SESSION 87 — Design Pipeline: Arena → Emporium → Marketplace + Crew Tables
## Bishop 027 | March 22, 2026
## Innovation Count: 1,935
## Based on: Session 70 spec (updated for K86 state)

---

## MISSION

Wire the creative economy pipeline. Designers upload work to the Arena. STAMP reviewers approve it. Approved work flows to the Emporium where businesses browse and buy. Design Battles auto-trigger when 2+ submissions land in the same category. The "you didn't lose" mechanic ensures every submission remains available — winners get the pot and trophy, everyone gets a permanent portfolio listing. Crew Tables let strangers assemble into project teams around Treasure Map objectives.

**Previous session**: K86 built Political Expedition (`/political-expedition`) — 5 tables, Google Civic API, 688 files deployed. Migration `20260322000017_political_expedition.sql` is the latest.
...
| Crew Call | `/crew-call` | ✅ LIVE with dispatch (K83) |
...
| Subscriptions | `/subscriptions` | ✅ LIVE (tiers + coalition + calculator) |
...
Create migration `20260322000018_design_pipeline.sql`:
...
Implementation: Do this as a client-side check after approval (in the STAMP review handler). No edge function needed.
...
### Route: `/crew-tables` (new page)

Create `src/pages/CrewTables.tsx`:

### 4A: Open Tables View

Cards showing: title, template type, seats filled/total, roles needed, stage, "Join" buttons. Each open role slot has a "Join as [Role]" button.

### 4B: Create Table Form
...
- Template selector (dropdown with presets):
  ...
  - **Event Launch**: Designer, Photographer, Coordinator, 2x Runners (3 of 5)
...
### 4D: Active Table View (Stage Tracker)
...
| `src/pages/CrewTables.tsx` | Crew Table browse + create + join + stage tracker |
| Migration `20260322000018_design_pipeline.sql` | arena_submissions + crew_tables + crew_table_seats |
...
- `makerSpotlightService.ts` — Spotlight rotation works
- `TreasureMapGame.tsx` — 52-card game is separate
- `Calendar.tsx` — Already wired K82
- `MoneyPenny*.tsx` — Already has AI K84
- `PoliticalExpedition.tsx` — Just built K86

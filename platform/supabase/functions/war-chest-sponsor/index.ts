...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: flagRow, error: flagErr } = await supabase
      ...
      .select("is_enabled")
      .eq("feature_key", "war_chest_sponsorship")
      .maybeSingle();
    ...
    console.error("[war-chest-sponsor]", error);
    return new Response(

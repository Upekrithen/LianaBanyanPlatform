-- ============================================================================
-- REPAIR MIGRATION: Create ALL tables from 20260307100000
-- ============================================================================
-- The original migration 20260307100000 was rolled back entirely because the
-- guilds table already existed (from Lovable initial setup) without leader_id.
-- This migration recreates ALL 26 tables idempotently.
-- ============================================================================
...
CREATE POLICY "Users can read own coverage account"
  ON coverage_minute_accounts FOR SELECT USING (auth.uid() = member_id);
...
CREATE POLICY "Users can update own coverage account"
  ON coverage_minute_accounts FOR UPDATE USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "System can insert coverage accounts" ON coverage_minute_accounts;
CREATE POLICY "System can insert coverage accounts"
  ON coverage_minute_accounts FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Users can read own transactions" ON coverage_minute_transactions;
CREATE POLICY "Users can read own transactions"
  ON coverage_minute_transactions FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "System can insert transactions" ON coverage_minute_transactions;
CREATE POLICY "System can insert transactions"
  ON coverage_minute_transactions FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Users can read donations they sent or received" ON coverage_minute_donations;
...
DROP POLICY IF EXISTS "Users can create donations from themselves" ON coverage_minute_donations;
CREATE POLICY "Users can create donations from themselves"
  ON coverage_minute_donations FOR INSERT WITH CHECK (auth.uid() = from_member_id);
...
DROP POLICY IF EXISTS "Users can read own donation views" ON donation_record_views;
CREATE POLICY "Users can read own donation views"
  ON donation_record_views FOR SELECT USING (auth.uid() = viewer_member_id);

DROP POLICY IF EXISTS "Users can create donation views" ON donation_record_views;
CREATE POLICY "Users can create donation views"
  ON donation_record_views FOR INSERT WITH CHECK (auth.uid() = viewer_member_id);
...
CREATE POLICY "Users can read own reading progress"
  ON reading_progress FOR SELECT USING (auth.uid() = member_id);
...
CREATE POLICY "Users can upsert own reading progress"
  ON reading_progress FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own reading progress" ON reading_progress;
CREATE POLICY "Users can update own reading progress"
  ON reading_progress FOR UPDATE USING (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Authenticated users can read round tables" ON round_tables;
CREATE POLICY "Authenticated users can read round tables"
  ON round_tables FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Moderators can create round tables" ON round_tables;
CREATE POLICY "Moderators can create round tables"
  ON round_tables FOR INSERT WITH CHECK (auth.uid() = moderator_id);

DROP POLICY IF EXISTS "Moderators can update their round tables" ON round_tables;
CREATE POLICY "Moderators can update their round tables"
  ON round_tables FOR UPDATE USING (auth.uid() = moderator_id);
...
DROP POLICY IF EXISTS "Authenticated users can read sessions" ON round_table_sessions;
CREATE POLICY "Authenticated users can read sessions"
  ON round_table_sessions FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "System can insert sessions" ON round_table_sessions;
CREATE POLICY "System can insert sessions"
  ON round_table_sessions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
DROP POLICY IF EXISTS "Authenticated users can read mic requests" ON mic_requests;
CREATE POLICY "Authenticated users can read mic requests"
  ON mic_requests FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users can create own mic requests" ON mic_requests;
CREATE POLICY "Users can create own mic requests"
  ON mic_requests FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own mic requests" ON mic_requests;
CREATE POLICY "Users can update own mic requests"
  ON mic_requests FOR UPDATE USING (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Authenticated users can read phase trunks" ON phase_mimictrunks;
CREATE POLICY "Authenticated users can read phase trunks"
  ON phase_mimictrunks FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Owners can update their phase trunks" ON phase_mimictrunks;
...
DROP POLICY IF EXISTS "Members can create personal phase trunks" ON phase_mimictrunks;
...
DROP POLICY IF EXISTS "Authenticated users can read validation attempts" ON phase_validation_attempts;
CREATE POLICY "Authenticated users can read validation attempts"
  ON phase_validation_attempts FOR SELECT USING (auth.uid() IS NOT NULL);
...
DROP POLICY IF EXISTS "Users can read own access records" ON phase_access_records;
CREATE POLICY "Users can read own access records"
  ON phase_access_records FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can insert own access records" ON phase_access_records;
CREATE POLICY "Users can insert own access records"
  ON phase_access_records FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Public pedestals visible to all authenticated users" ON pedestals;
...
DROP POLICY IF EXISTS "Curators can create pedestals" ON pedestals;
CREATE POLICY "Curators can create pedestals"
  ON pedestals FOR INSERT WITH CHECK (auth.uid() = curator_member_id);

DROP POLICY IF EXISTS "Curators can update own pedestals" ON pedestals;
CREATE POLICY "Curators can update own pedestals"
  ON pedestals FOR UPDATE USING (auth.uid() = curator_member_id);
...
DROP POLICY IF EXISTS "Users can read own contributions" ON pedestal_contributions;
CREATE POLICY "Users can read own contributions"
  ON pedestal_contributions FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can create own contributions" ON pedestal_contributions;
CREATE POLICY "Users can create own contributions"
  ON pedestal_contributions FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Authenticated users can read active feeds" ON subscription_feeds;
CREATE POLICY "Authenticated users can read active feeds"
  ON subscription_feeds FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Members can add feeds" ON subscription_feeds;
CREATE POLICY "Members can add feeds"
  ON subscription_feeds FOR INSERT WITH CHECK (auth.uid() = added_by_member_id);
...
DROP POLICY IF EXISTS "Users can read own private subscriptions" ON private_portfolio_subscriptions;
CREATE POLICY "Users can read own private subscriptions"
  ON private_portfolio_subscriptions FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can manage own subscriptions" ON private_portfolio_subscriptions;
CREATE POLICY "Users can manage own subscriptions"
  ON private_portfolio_subscriptions FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own subscriptions" ON private_portfolio_subscriptions;
CREATE POLICY "Users can update own subscriptions"
  ON private_portfolio_subscriptions FOR UPDATE USING (auth.uid() = member_id);

-- ── 16. GUILDS (already exists, add missing columns) ────────────────────

ALTER TABLE guilds ADD COLUMN IF NOT EXISTS leader_id UUID REFERENCES auth.users(id);
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS officer_ids UUID[] NOT NULL DEFAULT '{}';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS member_ids UUID[] NOT NULL DEFAULT '{}';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS member_count INTEGER NOT NULL DEFAULT 0;
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS tribe_ids UUID[] NOT NULL DEFAULT '{}';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS monthly_phase_fee NUMERIC(12,2) NOT NULL DEFAULT 100;
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS keep_ids TEXT[] NOT NULL DEFAULT '{}';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS banner_image_url TEXT;
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS rules_document TEXT;
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS ledger_section_id TEXT NOT NULL DEFAULT '';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS motto TEXT;
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active';
ALTER TABLE guilds ADD COLUMN IF NOT EXISTS phase_mimictrunk_id UUID;

ALTER TABLE guilds ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Authenticated users can read guilds" ON guilds;
CREATE POLICY "Authenticated users can read guilds"
  ON guilds FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Leaders can create guilds" ON guilds;
CREATE POLICY "Leaders can create guilds"
  ON guilds FOR INSERT WITH CHECK (auth.uid() = leader_id);

DROP POLICY IF EXISTS "Leaders can update their guilds" ON guilds;
...
DROP POLICY IF EXISTS "Authenticated users can read tribes" ON tribes;
CREATE POLICY "Authenticated users can read tribes"
  ON tribes FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Leaders can create tribes" ON tribes;
CREATE POLICY "Leaders can create tribes"
  ON tribes FOR INSERT WITH CHECK (auth.uid() = leader_id);

DROP POLICY IF EXISTS "Leaders can update their tribes" ON tribes;
CREATE POLICY "Leaders can update their tribes"
  ON tribes FOR UPDATE USING (auth.uid() = leader_id);
...
DROP POLICY IF EXISTS "Authenticated users can read guild memberships" ON guild_memberships;
CREATE POLICY "Authenticated users can read guild memberships"
  ON guild_memberships FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users can create own guild memberships" ON guild_memberships;
CREATE POLICY "Users can create own guild memberships"
  ON guild_memberships FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own guild memberships" ON guild_memberships;
CREATE POLICY "Users can update own guild memberships"
  ON guild_memberships FOR UPDATE USING (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Authenticated users can read tribe memberships" ON tribe_memberships;
CREATE POLICY "Authenticated users can read tribe memberships"
  ON tribe_memberships FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users can create own tribe memberships" ON tribe_memberships;
CREATE POLICY "Users can create own tribe memberships"
  ON tribe_memberships FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Users can read own packages" ON source_distribution_packages;
CREATE POLICY "Users can read own packages"
  ON source_distribution_packages FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can create own packages" ON source_distribution_packages;
CREATE POLICY "Users can create own packages"
  ON source_distribution_packages FOR INSERT WITH CHECK (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Users can read own handshakes" ON connection_handshakes;
CREATE POLICY "Users can read own handshakes"
  ON connection_handshakes FOR SELECT USING (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Active puzzles visible to authenticated users" ON real_world_puzzles;
...
DROP POLICY IF EXISTS "Members can create puzzles" ON real_world_puzzles;
CREATE POLICY "Members can create puzzles"
  ON real_world_puzzles FOR INSERT WITH CHECK (auth.uid() = creator_member_id);

DROP POLICY IF EXISTS "Creators can update own puzzles" ON real_world_puzzles;
CREATE POLICY "Creators can update own puzzles"
  ON real_world_puzzles FOR UPDATE USING (auth.uid() = creator_member_id);
...
DROP POLICY IF EXISTS "Users can read own puzzle attempts" ON puzzle_attempts;
CREATE POLICY "Users can read own puzzle attempts"
  ON puzzle_attempts FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can create own attempts" ON puzzle_attempts;
CREATE POLICY "Users can create own attempts"
  ON puzzle_attempts FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own attempts" ON puzzle_attempts;
CREATE POLICY "Users can update own attempts"
  ON puzzle_attempts FOR UPDATE USING (auth.uid() = member_id);
...
DROP POLICY IF EXISTS "Users can read own plane entries" ON plane_entries;
CREATE POLICY "Users can read own plane entries"
  ON plane_entries FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can create own plane entries" ON plane_entries;
CREATE POLICY "Users can create own plane entries"
  ON plane_entries FOR INSERT WITH CHECK (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can update own plane entries" ON plane_entries;
CREATE POLICY "Users can update own plane entries"
  ON plane_entries FOR UPDATE USING (auth.uid() = member_id);

-- ── 25. GOVERNANCE EVENTS ───────────────────────────────────────────────
...
DROP POLICY IF EXISTS "Authenticated users can read governance events" ON governance_events;
CREATE POLICY "Authenticated users can read governance events"
  ON governance_events FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Members can insert governance events" ON governance_events;
CREATE POLICY "Members can insert governance events"
  ON governance_events FOR INSERT WITH CHECK (auth.uid() = actor_member_id);
...
DROP POLICY IF EXISTS "Users can read own deck card links" ON special_deck_card_links;
CREATE POLICY "Users can read own deck card links"
  ON special_deck_card_links FOR SELECT USING (auth.uid() = member_id);

DROP POLICY IF EXISTS "Users can insert own deck card links" ON special_deck_card_links;
CREATE POLICY "Users can insert own deck card links"
  ON special_deck_card_links FOR INSERT WITH CHECK (auth.uid() = member_id);

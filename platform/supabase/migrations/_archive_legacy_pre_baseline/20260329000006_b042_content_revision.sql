-- Migration: 20260329000006_b042_content_revision.sql
...
-- cost-plus-twenty (article): Cost + 20%: The Constitutional Floor
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
**Legal Notice:** Liana Banyan Corporation (Wyoming C-Corp, EIN 41-2797446) operates under a constitutionally locked Cost+20% margin. Credits are prepaid service access within a closed-loop cooperative system. They cannot be withdrawn as cash or exchanged for external currency. Marks are transactional byproducts of the pricing formula, not securities, equity, or investment instruments (see SEC v. Ripple Labs "Other Distributions" holding; Teamsters v. Daniel, 439 U.S. 551). 83.3% of every transaction flows to the creator or service provider.

---
$b042rev$,
    updated_at = now()
WHERE slug = 'cost-plus-twenty';

-- three-currency-system (article): The Three-Currency System
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
**Legal Notice:** Credits, Marks, and Joules are platform service currencies within a closed-loop cooperative system. They are not securities, equity, cryptocurrency, tokens, or investment instruments. They cannot be withdrawn as cash or exchanged for external currency. Marks represent earned participation or effort-debt, not financial debt. Joules represent stored cooperative value, not financial returns. Liana Banyan does not use blockchain technology. The Currency Differential is the subject of Patent Bag 54 (provisional filing).

---
$b042rev$,
    updated_at = now()
WHERE slug = 'three-currency-system';

-- cold-start-pathways (article): Six Cold Start Pathways
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
$b042rev$,
    updated_at = now()
WHERE slug = 'cold-start-pathways';

-- captain-system (article): The Captain System
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
$b042rev$,
    updated_at = now()
WHERE slug = 'captain-system';

-- moneypenny-gatekeeper (article): MoneyPenny: AI Gatekeeper
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
$b042rev$,
    updated_at = now()
WHERE slug = 'moneypenny-gatekeeper';

-- lb-card (article): The LB Card
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
**Legal Notice:** The LB Card is a prepaid card issued through Stripe Issuing, subject to Stripe's cardholder terms and applicable card network rules. Liana Banyan Corporation facilitates card funding through platform-mediated transactions. The LB Card is not a bank account, credit card, or line of credit. Funds are FDIC-insured through Stripe's partner bank. Daily funding limits and transaction limits apply. Credits cannot fund LB Cards — the one-way valve is permanent.
...
-- project-entity-architecture (article): Project-Entity Architecture
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
...
$b042rev$,
    updated_at = now()
WHERE slug = 'project-entity-architecture';

-- adapt-score-system (system_design): ADAPT Score: The Instrument Panel
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
### System Overview
...
$b042rev$,
    updated_at = now()
WHERE slug = 'adapt-score-system';

-- star-chamber-verification (system_design): Star Chamber: Multi-Agent Verification
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
### System Overview
...
$b042rev$,
    updated_at = now()
WHERE slug = 'star-chamber-verification';

-- waterwheels-economic-engine (system_design): WaterWheels: The Economic Engine
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
### System Overview
...
$b042rev$,
    updated_at = now()
WHERE slug = 'waterwheels-economic-engine';

-- design-pipeline-architecture (system_design): Design Pipeline: Arena + Emporium + Crew Tables
UPDATE cephas_content_registry
SET content_markdown = $b042rev$
### System Overview

...
const POSITION_CATEGORIES = [
  { value: "create_idea" as const, label: "Ideation & Creation", icon: TrendingUp },
  { value: "define_describe_document" as const, label: "Documentation", icon: Briefcase },
  { value: "research_development" as const, label: "Research & Development", icon: Building },
  { value: "prototype" as const, label: "Prototyping & Engineering", icon: Building },
  { value: "legal_services" as const, label: "Legal Services", icon: Briefcase },
  { value: "logistics_blockchain" as const, label: "Logistics & Blockchain", icon: Building },
  { value: "steward_owner" as const, label: "Stewardship & Leadership", icon: Building },
  { value: "marketing_services" as const, label: "Marketing & Sales", icon: TrendingUp },
  { value: "accounting_services" as const, label: "Accounting & Finance", icon: DollarSign },
  { value: "hr_staffing" as const, label: "HR & Staffing", icon: Briefcase },
  { value: "materials_sourcing" as const, label: "Materials & Sourcing", icon: Building },
  { value: "manufacture_assembly" as const, label: "Manufacturing & Assembly", icon: Building },
  { value: "kickstarter_campaign" as const, label: "Crowdfunding Campaign", icon: TrendingUp },
  { value: "it_services" as const, label: "IT Services", icon: Building },
  { value: "delivery" as const, label: "Delivery & Distribution", icon: Building }
];
...
export default function AllPositionsBrowse() {
  ...
  const { data: positions, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (searchTerm) {
        query = query.or(`position_title.ilike.%${searchTerm}%,position_description.ilike.%${searchTerm}%`);
      }
                ...
                <Card key={position.id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        className="flex-1"
                        onClick={() => navigate(`/projects/${(position.project as any)?.id}/positions`)}
                      >
                        View Details
                      </Button>
                      <Button
                        variant="outline"
                        onClick={() => navigate(`/positions/${position.id}/apply`)}
                      >
                        Apply
                      </Button>
                    </div>

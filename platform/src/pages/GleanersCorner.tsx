...
function SplitBar() {
  ...
  return (
    <div ref={barRef} className="space-y-4">
      {/* The bar */}
      <div className="relative h-16 rounded-xl overflow-hidden flex bg-slate-800/50 border border-slate-700/50">
        {segments.map((seg, i) => (
          <div
            key={seg.key}
      ...
      <div className="grid grid-cols-3 gap-4">
        {segments.map((seg) => (
          ...
          >
            ...
            <div className="text-xs text-slate-400 mt-1">{seg.description}</div>
          </div>
        ))}
      </div>
      ...
      {hoveredSegment && (
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-lg p-4 animate-in fade-in duration-200">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm text-slate-400 whitespace-nowrap">Product Price:</label>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="border-blue-500/20 bg-blue-500/5">
          <CardContent className="pt-6">
            <div className="text-sm text-slate-400 mb-1">Total Distributed</div>
        ...
        <Card className="border-slate-500/20 bg-slate-500/5">
          <CardContent className="pt-6">
            <div className="text-sm text-slate-400 mb-1">Reserve Balance</div>
      ...
      <Card className="border-slate-700/50">
        <CardHeader>
          <CardTitle className="text-lg text-white">Distribution by Category</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {CATEGORY_BREAKDOWN.map((cat) => (
              <div key={cat.category} className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-300 w-36">{cat.label}</span>
                ...
                <span className="text-sm font-medium text-slate-400 w-10 text-right">
                  {cat.percentage}%
                </span>
              </div>
            ))}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <th className="text-right py-2 px-3 text-slate-400 font-medium">Recipients</th>
                  <th className="text-right py-2 px-3 text-slate-400 font-medium">Amount</th>
                </tr>
              </thead>
              <tbody>
                {SAMPLE_DISTRIBUTIONS.map((dist) => (
                  <tr key={dist.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="py-2.5 px-3 text-slate-300">{dist.description}</td>
                    <td className="py-2.5 px-3 text-right text-slate-300">{dist.recipientCount}</td>
...
function HowItWorks() {
  return (
    <Accordion type="single" collapsible className="space-y-2">
      <AccordionItem value="automatic" className="border border-slate-700/50 rounded-lg px-4">
      ...
      <AccordionItem value="where" className="border border-slate-700/50 rounded-lg px-4">
        <AccordionTrigger className="text-white hover:no-underline">
          <div className="flex items-center gap-3">
        ...
        <AccordionContent className="text-slate-400">
          <ul className="space-y-2">
            <li className="flex items-start gap-2">
              ...
              <span><strong className="text-white">Food assistance</strong> — Mark-funded grocery essentials distributed to families in need</span>
            </li>
            <li className="flex items-start gap-2">
              <Stethoscope className="h-4 w-4 mt-0.5 text-blue-400 shrink-0" />
              <span><strong className="text-white">Medical Savings (MSA)</strong> — Dental, prescriptions, specialist visits</span>
            </li>
            <li className="flex items-start gap-2">
              <AlertTriangle className="h-4 w-4 mt-0.5 text-red-400 shrink-0" />
              <span><strong className="text-white">Emergency support</strong> — Urgent utilities, transportation, crisis assistance</span>
            </li>
          </ul>
        </AccordionContent>
      </AccordionItem>

      <AccordionItem value="why" className="border border-slate-700/50 rounded-lg px-4">
        <AccordionTrigger className="text-white hover:no-underline">
          <div className="flex items-center gap-3">
        ...
        <AccordionContent className="text-slate-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/30">
              <div className="text-xs text-slate-500 mb-1">At $1M monthly volume</div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/30">
              <div className="text-xs text-slate-500 mb-1">At $10M monthly volume</div>
              <div className="text-lg font-bold text-emerald-400">
      ...
      <AccordionItem value="ruth" className="border border-slate-700/50 rounded-lg px-4">
        <AccordionTrigger className="text-white hover:no-underline">
          <div className="flex items-center gap-3">
      ...
      <AccordionItem value="marks" className="border border-slate-700/50 rounded-lg px-4">
        <AccordionTrigger className="text-white hover:no-underline">
          <div className="flex items-center gap-3">
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white">
      <div className="container max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm italic text-slate-500 max-w-lg mx-auto">

...
- REAL GPQA Diamond (~448q, Idavidrein/gpqa) NOT YET downloaded -- HF auth required
...
- **Mesh test infrastructure:** MMLU-Pro 12K + MMLU-Pro Diamond 2.8K (internal high-difficulty subset) downloaded, all tooling built, blocked on IPC fix + M2/M3 installs. NOTE: the 2.8K "GPQA Diamond" label in prior drafts was WRONG -- that file is the MMLU-Pro Diamond internal subset, not the real GPQA dataset (Idavidrein/gpqa, ~448q, HF-gated).
